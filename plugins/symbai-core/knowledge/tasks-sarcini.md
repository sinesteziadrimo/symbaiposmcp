...
- **Sarcina recurentă nu apare azi.** Instanțele zilei se generează **la prima deschidere** a listei de sarcini (`/my-tasks`, dashboardul de sarcini sau `get_my_tasks`) — nu în avans, peste noapte. Practic: dacă nimeni nu deschide sarcinile într-o zi, instanțele acelei zile nu se creează, iar retroactiv **nu** se generează. Dacă tot lipsește, verifică dacă lista e activă și dacă recurența se potrivește zilei (ex. `weekly` cu zilele corecte).
  - **Ce înseamnă asta în practică**, dacă ții evidența pentru control (HACCP, ISU, ANSVSA): deschide sarcinile în fiecare zi de lucru, măcar o dată, ca ziua respectivă să aibă urmă. Cel mai simplu: pune-le pe primul ecran al rolului care începe tura (Symbai Staff → fila „Azi"), ca deschiderea să se întâmple de la sine.
