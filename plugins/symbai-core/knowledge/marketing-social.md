...
- `genereaza_link_conectare` — [marketing_social] Facebook/TikTok: link stabil către Conturi Social Media pe brand, autorizare în browser; Facebook cere apoi alegerea explicită a paginii în Symbai, păstrând accesul paginilor celorlalte branduri. Celelalte platforme primesc link OAuth temporar. `conecteaza_instagram_din_facebook(brandId,expectedFacebookPageId)` leagă IG din pagina FB confirmată. Verifică identitatea prin `verifica_integrare` cu același ID dorit. Workflow complet în skill-ul `conecteaza-meta`.
