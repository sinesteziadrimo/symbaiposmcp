...
9. **Reîmprospătezi articolele vechi (igienă lunară — prospețimea e semnal-cheie și pentru citarea în AI)**: `list_blog_refresh_candidates` → primești articolele neactualizate de peste 90 de zile sau cu trafic în declin (când Search Console e conectat) → pentru fiecare, actualizezi SUBSTANȚIAL cu `update_blog_post` (prețuri actuale, meniul curent, statistici noi, o secțiune nouă) — NU doar re-salvezi. Data modificării și sitemap-ul se actualizează automat la salvare; interfața Blog încearcă trimiterea IndexNow best-effort, iar scrierile MCP nu o declanșează încă.

### Ce face platforma AUTOMAT la publicare (nu mai repeta manual)

- Articolul primește **pagină randată pe server**, cu titlu, meta, date structurate (schema de articol) și conținutul complet în HTML — vizibil și pentru Google, și pentru roboții AI (ChatGPT/Claude/Perplexity), care NU execută JavaScript. Nu adăuga schema manual în conținut.
- Intră automat în **sitemap.xml, RSS și llms.txt** (se împrospătează în ~10 minute). Publicarea/editarea din interfața Blog **încearcă trimiterea IndexNow best-effort** (Bing și servicii compatibile; Google descoperă prin sitemap); scrierile MCP nu o declanșează încă.
- Bifă „**indexable = false**" = noindex real pe pagină + exclus din sitemap (pentru advertoriale/teste).
- **Slug-urile rezervate aplicației** (tracker, posts, authors, import, redirects, analytics, migration, advertorials, backlinks) sunt respinse — alege alt slug dacă primești eroarea.
- La publicare sunt obligatorii **meta description ≥70 caractere** și **imagine de copertă** — pregătește-le înainte să setezi starea „Publicat".
...
- **De ce nu apare articolul în Google deși e publicat?** Indexarea durează de la câteva zile la 1-2 săptămâni. Platforma actualizează sitemap-ul și randează pagina pe server; interfața Blog încearcă trimiterea IndexNow best-effort, iar scrierile MCP nu o declanșează încă. Tu asigură-te că articolul are H2-uri, link-uri interne și meta bună. Dacă e urgent, cere inspecția URL-ului în Search Console (Conectat din SEO → Setări).
