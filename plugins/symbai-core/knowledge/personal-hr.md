...
## ⚠ Capcane MCP confirmate (testate live, 2026-06)

Reguli care te scapă de greșeli pe care utilizatorul le-ar vedea DIRECT. Citește-le înainte de scrieri pe Personal.

1. **Angajat „invizibil" în Lista Personal.** `create_employee`/`update_employee` setează doar `brand_id`/`location_id` singular și lasă `brand_ids[]`/`location_ids[]` GOALE; iar directorul UI filtrează angajații non-admin după **array** (`brand_ids`). Rezultat: pe unitatea lui, angajatul creat prin MCP NU apare în listă (apare doar pe „Toate unitățile"). Nu există param MCP pentru array. → După creare: confirmă prin `get_staff_overview(brandId)` (acolo apare), iar userului spune-i să-l vadă pe „Toate unitățile" sau să deschidă+salveze fișa în aplicație o dată.
...
3. **`create_role` cu `permissions` ca OBIECT = rol stricat.** Salvează blob-ul `{pos:true}` ca atare (nu chei reale) și apoi `set_role_permissions(addPermissions)` crapă cu „n is not iterable" (încearcă să itereze un obiect). → Creează rolul fără `permissions`, apoi `set_role_permissions(roleId, permissions:[...])` (array). Forma `addPermissions`/`removePermissions` merge doar când setul curent e deja array.
...
7. **`configure_pos_settings` e stricat acum** — întoarce eroare SQL („syntax error at or near where"). Pentru setări POS, îndrumă userul în UI până se repară.
...
## Catalog permisiuni (chei reale — sursă: `shared/permissions.ts`)
