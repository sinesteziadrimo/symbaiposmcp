...
- **PIN** — cod de 4 cifre pentru autentificare rapidă pe POS și pontare. Câmpul PIN apare pe fișa angajatului doar dacă rolul lui are permisiunea de logare cu PIN. Pe telefonul personal, Symbai Staff nu cere PIN din nou la fiecare operație permisă; PIN-ul operațional este pentru Workstation Tablet partajat. Prima asociere/reasociere a telefonului Staff cere parola o singură dată.
...
- `get_staff_overview` — personal, roluri, programări, ture POS, alocări de producție și pontaje, per brand/locație. Verifică perioada și paginarea; un rezumat sau prima pagină nu sunt inventarul complet.
...
- Tura POS și programarea sunt evidențe separate. `create_shift`/`update_shift` lucrează cu tura POS; `create_staff_schedule`/`update_staff_schedule` cu Planificatorul de Ture. Programarea acceptă `brandId`, `locationId`, `floorConfigId`, `sectionId` și `sectionName`, inclusiv la crearea în masă. Pentru o corecție păstrează ID-ul programării existente. Aranjamente de sală + raioane: `create_floor_zone`, `bulk_create_floor_tables`, `create_floor_config`, `add_sections_to_config`, `assign_tables_to_section`, `create_floor_config_schedule` (`dayOfWeek` 0=Duminică…6=Sâmbătă).
...
- Sarcini: `create_targeted_task_list` + `create_targeted_task` acceptă rol, tură, raion, recurență și dovadă. Citire: `list_task_lists`/`list_tasks`/`get_task`/`get_task_dashboard`/`get_my_tasks`; editare: `update_task_list`/`clone_task_list`/`update_task`/`complete_task`/`assign_task`. Pentru `targetSection` verifică raionul programului publicat al angajatului. Îl poți seta cu `create_staff_schedule` sau `update_staff_schedule`, apoi verifici destinatarii efectivi. Vezi `knowledge/tasks-sarcini.md` + skill-ul `gestioneaza-sarcini`.

Atenție: turele de **producție** (fabrică) sunt alt concept și au tool-urile lor în modulul **productie** — descoperă schema live a turelor și a asignării operatorilor. Pontajele efective din Staff și kiosk sunt, de asemenea, distincte de program. `delete_staff_schedule` este disponibil pentru ștergerea unei programări, cu previzualizare și acord; nu îl folosi pentru a șterge ori fabrica prezența reală.

Pentru o săptămână completă, citește întâi schema live `get_staff_overview`. Dacă sunt disponibile, trimite `dateFrom`/`dateTo` (date incluse, maximum 93 de zile), `query` sau `employeeId`, și `collection`: `employees`, `scheduledRotaDetails`, `upcomingShiftDetails`, `productionScheduledRotaDetails`, `selfServiceAttendanceDetails` ori `kioskAttendanceDetails`. Urmează `pagination.nextArguments` până la `hasMore:false`. `all` cu detalii întoarce pagini mai mici; selectarea unei colecții permite pagini mai mari. Concediile se verifică separat cu `list_leave_requests`. Pe versiuni mai vechi, verifică parametrii existenți și totalurile, folosește pagini mici și nu pretinde acoperirea unor date sau detalii pe care răspunsul nu le conține. `complete:false`/`dataOmitted` cere o citire mai restrânsă, nu presupuneri despre rândurile lipsă.
...
1. **Verifică selecția înainte să schimbi fișa.** Lipsa atribuirii de brand înseamnă toate brandurile; lipsa locației înseamnă toate locațiile, independent. Nu pune o restricție artificială pentru a face angajatul vizibil. Verifică filtrul, toate paginile, starea activă și aria utilizatorului. După creare recitește fișa și unitățile salvate.
...
5. **Raionul se poate seta prin MCP în Planificator.** Trimite `sectionId`/`sectionName` la `create_staff_schedule` sau `update_staff_schedule`; păstrează numele identic cu raionul meselor. Programarea, tura POS și prezența efectivă se verifică separat.
6. **Orele programării sunt locale.** `scheduledStart:"2026-09-14T10:00"` cere 10:00 în România. Nu scădea offset-ul și nu marca ora locală cu `Z`. Dacă folosești ISO cu offset, acesta trebuie să fie corect pentru data cerută. Pentru o programare de noapte trimite sfârșitul în ziua următoare și recitește orele în `Europe/Bucharest`.
