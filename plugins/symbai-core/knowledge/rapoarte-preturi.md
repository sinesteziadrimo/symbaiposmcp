...
- **COGS (costul mărfii vândute)** — în P&L e **realizat**: calculat din loturile chiar consumate, la prețurile lor reale de intrare (la ieșire pleacă primul lotul care expiră cel mai devreme; la termene egale sau fără termen, cel intrat primul), cu revenire pe costul teoretic din rețetă acolo unde lipsește consumul. Recalcularea consumului se reflectă în rapoarte. Două precizări de reținut înainte de a trage concluzii:
  - **Livrările (Glovo/Wolt/Bolt/Tazz) descarcă imediat loturile reale** când comanda este finalizată, deci costul realizat vine din loturile consumate. O corecție de preț pe o factură veche ajunge și la livrările finalizate când reprocesarea acoperă intervalul lor; recalcularea **pe produs** din pagina Consum Zilnic este doar varianta mai îngustă.
...
2a. **P&L-ul refuză cu 409 / integritate incompletă** — nu transforma eroarea în zero și nu lua decizii dintr-un total parțial. Rulează `diagnose_pnl_integrity` pe aceeași perioadă și același brand/locație: întoarce blocajele exacte și, numai pentru diagnostic, un preview marcat explicit incomplet. Repară sursa indicată, apoi repetă `get_pnl`; raportul strict rămâne intenționat blocat până când cifrele sunt complete.
   - Nu presupune că „tipurile de produs nelegate la categorii P&L" sunt cauza unui 409. O legătură lipsă poate trimite sume la **Nealocate**, dar blocajul strict trebuie atribuit numai textului întors în `blockingIssues`.
   - Dacă blockerul spune că produse fizice nu au **cost FIFO realizat**, caută produsele vândute și ingredientele/loturile sursă cu cost lipsă sau zero. Corectează costul istoric demonstrabil al recepției/lotului; nu inventa costul din prețul curent al produsului și nu folosi categoria P&L ca substitut.
   - După corectarea loturilor, reprocesarea consumului pe interval este pasul care reconstruiește COGS-ul istoric. Înainte de orice reprocesare, rulează diagnosticul de rutare pe gestiuni și oprește-te dacă există ingrediente fără gestiune sau rutate în alt local; apoi repetă `diagnose_pnl_integrity` până la `status=complete`.
...
5. **Verific închiderea zilei** — `/finance/end-of-day` pe ziua dorită: compari totalul Z fiscal cu totalul POS, verifici variația de la numărarea casei și anulările.
   - Pentru **Vânzări Angajați**, angajatul este atribuit din comenzile închise în exact aceeași fereastră de business selectată. Nu muta automat vânzarea pe data turei. Dacă raportul pare să includă o altă zi, compară comenzile, tura, jurnalul și sfârșitul de zi pe aceeași locație și aceleași limite orare.
...
- `generate_report` — raport rapid: `food_cost`, `sales_summary` sau `stock_value`. Pentru `stock_value`, același utilizator trebuie să aibă cumulativ dreptul de a vedea **rapoarte**, **costuri** și **inventarul în regim de citire**; lipsa unuia dintre ele oprește raportul. Rezultatul separă clar produsele fără cost, produsele fără preț de vânzare și pozițiile cu stoc negativ — nu le ascunde și nu le prezenta ca zero.
...
- **„De ce diferă food cost-ul din rețetă de cel din P&L?"** — P&L folosește COGS **realizat** (loturile chiar consumate, la prețurile lor reale de intrare); rețeta dă costul teoretic. Diferența e normală (risipă, porționare, prețuri de achiziție diferite pe loturi) și e ea însăși un KPI (variația food cost teoretic vs real). Două precizări de spus explicit înainte de a căuta o greșeală: **livrările** (Glovo/Wolt/Bolt/Tazz) descarcă imediat loturile reale, iar reprocesarea intervalului include comenzile finalizate din perioada aleasă; **produsele oferite (protocol, consum intern) și cele transferate** consumă stoc fără să aducă venit și umflă procentul. Explicația completă a mecanismului e în `consum-zilnic-cost-marfa.md`.
...
- **„Am ales 24 iulie 06:00 → 25 iulie 06:00 și apar vânzări din seara de 25"** — intervalul personalizat este exact, cu capătul final exclusiv; nu trebuie extins până pe 26. Confirmă fusul orar al organizației, locația și orele afișate, apoi compară comenzile închise și jurnalul pe aceeași fereastră. Presetările de zile sunt inclusive, intervalele orare explicite nu sunt.
