...
2d. **Verific livrările separat** — `list_delivery_pnl_segments` → `get_delivery_pnl(configId, perioada)`. Dacă nu există segment salvat, `get_delivery_pnl(brandId)` e doar calcul ad-hoc venit−marfă; pentru manoperă + cheltuieli configurate și comparația vizuală cu perioada precedentă deschide `/reports/pnl-livrari`.
