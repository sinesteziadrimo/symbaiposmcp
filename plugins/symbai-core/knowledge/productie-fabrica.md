...
Dacă userul întreabă „desenează-mi hala", „unde pun utilajele/magaziile pe plan", „vreau planul fabricii" sau „arată-mi fluxul fizic prin hală", folosește skill-ul `plan-fabrica` și pagina **Plan Fabrică 2D** (`/factory-floor-plan`). Este un editor vizual dedicat fabricii, separat de Plan Sală: planul are nivele/etaje, obiecte cu geometrie în centimetri și conexiuni de tip flux material / conveyor / personal / utilități. Obiectele pot fi legate de entități reale: `production_equipment`, `production_zone`, `warehouse`, `storage_zone`; mutarea pe canvas schimbă doar geometria planului, nu modifică echipamentul/magazia reală.

Workflow agent (MCP-first — EXISTĂ acum tool-uri dedicate de plan, nu mai e nevoie să desenezi doar din browser):
1. Creează/verifică întâi entitățile reale prin MCP: `create_production_zone`, `create_production_equipment`, magazii și zone de depozitare (`create_warehouse`, `bulk_create_storage_zones`) sau paginile de inventar. Planul nu înlocuiește configurarea reală.
2. Creează planul: `create_factory_plan` (nume + locație). Vezi ce poți pune: `get_factory_plan_palette` (listează zonele, echipamentele, magaziile și zonele de depozitare reale + care sunt deja pe plan).
...
4. Citește oricând `get_factory_plan` — întoarce obiectele + conexiunile + date LIVE: statusul fiecărui echipament, fluxurile care îl folosesc și câte operații are azi, plus stocul real pe fiecare zonă de depozitare și agregat pe magazie. Pentru cifre decizionale verifică și prin tool-urile de stoc/producție.
...
6. Explică simplu: „Am pus cuptorul, zona de ambalare și magazia pe plan și am legat fluxul; stocul real se vede pe zonele de depozitare, iar utilajele arată pe ce fluxuri lucrează."
...
## Pentru acces SQL
Tabele relevante: `production_batches` (loturi/șarje), `inventory_lots` (loturi de stoc), `genealogy_edges` (trasabilitate intrare→ieșire), `lot_outbound_traces` (urmă lot→document de ieșire pentru recall un-pas-înainte către client), `production_containers` (containere cu QR unic), `batch_material_links` (loturi alocate explicit pe șarjă; esențial pentru staging/pegging real), `operation_executions` (execuții shop-floor), `planned_lots` (loturi planificate), `quality_hold_events` (blocaje QC), `production_qc_inspections` (inspecții QC), `capa_actions` (CAPA/NCR), `mps_schedule_entries` (plan MPS), `production_zones`/`production_equipment`/`equipment_recipe_capacities`/`zone_ingredient_warehouses` (infrastructură), `factory_floor_plans`/`floor_objects`/`floor_object_connections` (planul 2D al halei; geometrie + legături vizuale către entități reale), `production_shifts` (ture), `haccp_sensors` (senzori), `inventory_documents` + liniile lor (documente consum/producție).
