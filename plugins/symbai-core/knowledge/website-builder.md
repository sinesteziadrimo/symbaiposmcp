...
- `set_website_theme` — identitatea vizuală globală (`config.global`): `primaryColor`, `secondaryColor`, `accentColor`, `backgroundColor`, `textColor`, `fontFamily`, `headingFont`, `siteName`, `tagline`, `darkMode`. Folosește-l când potrivești site-ul cu brandul real; nu setează `customCss`.
- `update_website_navigation` — meniul de sus. `rebuildCategoriesFromCatalog: true` reconstruiește dropdown-ul „Categorii" din categoriile reale **rădăcină** (scoate categoriile generice de template). Sau dă `items[]` pentru control total. Pentru chrome-ul navbar trimite `navbarSettings` (ex. `ctaButton:{visible:false}`, `showSearch:false`, `showLoginButton:false`, `transparent:true`, `sticky:true`, `logoText`).
