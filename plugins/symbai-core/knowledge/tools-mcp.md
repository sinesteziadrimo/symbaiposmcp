...
- `build_factory_floor` — COMPOZIT: construiește o hală dintr-o singură comandă: obiecte reale/generice + conexiuni. Pentru operator folosește `objectType:"operator"` + `entityName`/`entityId` de angajat (MCP îl mapează la `employee`); poți seta `icon` ca emoji sau URL imagine. (necesită: planId, objects)
...
- `place_factory_object` — Pune UN obiect pe plan, legat de o entitate reală (entityId SAU entityName) sau generic (wall/door/aisle/custom). Pentru operator: `objectType:"operator"` + angajat prin `entityName`/`entityId` (MCP îl mapează la `employee`). `icon` poate fi emoji sau URL imagine. (necesită: planId, objectType)
