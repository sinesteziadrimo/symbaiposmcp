...
- `update_website_navigation` aplica direct `navbarSettings` pentru stil/logo/search/login/CTA/sidebar; foloseste-l in loc de fallback DB/config pentru chrome-ul navbar. Campuri noi utile: `logoUrlDark` pentru logo pe pagini light/solide, `navbarBg` pentru bara solida cream/branded, `ctaButton.style:"outline"` pentru CTA contur, `ctaButton.color:"#c9a45c"` pentru text+bordura CTA outline custom (ex. auriu pe dark), `ctaButton.bgColor` pentru fundalul pill-ului solid (`color` devine textul), `transparent:true` pentru overlay peste hero doar pe pagini hero-first, `showCurrency` si `currencies` pentru selector valuta.
- `list_website_component_catalog`/`set_website_page_content` pot folosi noile config-uri native: `hero-slider.heroLayout:"diagonal-split"`, `slides[].titleAccent`, `hero-slider.titleStroke` + `titleStrokeWidth`, `feature-cards.style:"divided"`, `cards[].description`, `cards[].linkText`, `feature-cards.titleAlign` + `\n` in titlu, `tabbed-cards` (`tabs[]`, `cards[].tab`, `showAllTab`), `split-list` (`image`, `imagePosition`, `items[].title/description/linkText/linkUrl`) pentru lista editoriala langa imagine, `content-carousel` (`items[].image/title/description/link`, `imageRatio`, `accentColor`, `dotColor`) pentru slider editorial inline, `process-steps` (`steps[].title/description/number`, `layout`, `numberStyle`, `connectors`) pentru "cum functioneaza"/pasi numerotati, `marquee` (`items[]`, `speed`, `separator`, `direction`, `textCase`, culori), `app-download` (`appStoreUrl`, `googlePlayUrl` sau `stores[]`, optional `image`) pentru badge-uri App Store / Google Play si `menu-list` pentru meniu restaurant static (nu POS live).
