...
- `list_website_component_catalog`/`set_website_page_content` pot folosi noile config-uri native: `hero-slider.heroLayout:"diagonal-split"`, `slides[].titleAccent`, `hero-slider.titleStroke` + `titleStrokeWidth`, `feature-cards.style:"divided"`, `cards[].description`, `cards[].linkText`, `feature-cards.titleAlign` + `\n` in titlu, `tabbed-cards` (`tabs[]`, `cards[].tab`, `showAllTab`), `split-list` (`image`, `imagePosition`, `items[].title/description/linkText/linkUrl`) pentru lista editoriala langa imagine, `content-carousel` (`items[].image/title/description/link`, `imageRatio`, `accentColor`, `dotColor`) pentru slider editorial inline, `marquee` (`items[]`, `speed`, `separator`, `direction`, `textCase`, culori), `app-download` (`appStoreUrl`, `googlePlayUrl` sau `stores[]`, optional `image`) pentru badge-uri App Store / Google Play si `menu-list` pentru meniu restaurant static (nu POS live).
