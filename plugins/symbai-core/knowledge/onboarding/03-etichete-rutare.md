...
- Citirile (`list_tags`, `list_tag_summary`, `list_untagged_products`, `list_menu_categories`, `search_products_db`...) sunt disponibile mereu, fără modul.
...
- **Confirmare înainte de fiecare scriere în masă**: *„Aplic eticheta «Bar» pe toate produsele din ramura BAR (~N produse, conform categoriilor). Confirmi?"* — N îl estimezi din numerele per categorie date de `list_menu_categories`.
...
**Pas 2 — asignează în masă** cu `bulk_assign_tag` (obligatoriu: `tagId`; trimite mereu și `brandId`). Filtrele se combină cu AND:
...
- [ ] `get_product_details(productId)` pe 1–2 produse-mostră din fiecare secție → etichetele apar pe fișă.
...
- **`search_products_for_tagging` și `delete_tag` NU există prin MCP** — sunt doar în agentul din aplicație. Nu ai dry-run pentru filtre; compensezi cu: numerele per categorie din `list_menu_categories` (estimare), idempotența lui `bulk_assign_tag` și reversibilitatea prin `bulk_remove_tag` cu aceleași filtre.
...
- **Produsele sunt globale pe firmă, nu per brand/locație** — eticheta pusă pe un produs îl urmează în toate locațiile. La clienți multi-locație, izolarea se face la regulile de rutare (per locație, în aplicație), nu la etichete; o etichetă „de barul X" pe produse vândute și în localul Y poate ruta bonuri la alt local dacă regula nu e limitată pe locație (incident real în producție). Semnalează asta când configurezi multi-locație.
