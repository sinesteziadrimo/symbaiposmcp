...
- **Casa de marcat fiscală: nu promite driverul nativ implicit.** Datecs are ca recomandare operațională **DUDE** (instalat automat de Print Agent) până la validarea completă a driverului serial Symbai pe casa fizică. Daisy/Tremol și alte mărci se tratează ca configurare avansată din Setări (Fisco/folder fiscal, EcrBridge sau nativ beta unde există validare) — nu promite că merg direct din wizard și nu le crea prin MCP ca funcționale fără test fizic.
