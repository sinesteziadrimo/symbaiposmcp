...
Pentru Glovo, explică pașii reali:
- se obțin de la Glovo: Partner API token, Glovo Store ID și Store Address External ID;
- pagina de integrări livrare afișează cele trei URL-uri publice de trimis la Glovo: dispatch webhook, cancellation webhook și Menu JSON URL;
- sincronizarea completă de meniu trimite la Glovo un `menuUrl` după validare, iar modificările mici de preț/disponibilitate merg prin actualizare în masă;
- auto-accept-ul inteligent Glovo folosește KDS-ul pentru ETA promis; dacă ETA trece peste limita maximă, comanda rămâne pentru operator;
- Symbai poate trimite accept/gata/predare către Glovo, dar nu promite refuz/anulare din Symbai: acestea se fac în aplicația/suportul Glovo, iar statusul se reflectă apoi automat înapoi.
...
- **Website-ul** — builder vizual cu asistent AI propriu în pagină (creezi site, alegi template pe profil, editezi pagini, publici cu Salvează). `gaseste_in_aplicatie("creează website")` (zona Magazin Online → Website-uri). Pozele se urcă întâi în Biblioteca Media (`gaseste_in_aplicatie("bibliotecă media brand")`) — nu există upload prin conexiune. Verificare: `list_entities(entityType: "menu_display_configs", brandId)` și caută intrarea cu profilul `website`, sau întreabă utilizatorul. Paginile site-ului sunt descrise în `marketing-social.md`.
- **Credențialele platformelor de livrare + sincronizarea meniului** — `gaseste_in_aplicatie("conectare Glovo Wolt manager canale")` → tab Integrări (datele de la platformă), apoi tab Meniu & Prețuri (sincronizare). Pentru Glovo, copiază către Glovo cele trei URL-uri afișate de Symbai (dispatch webhook, cancellation webhook și Menu JSON URL); sincronizarea completă de meniu folosește `menuUrl`, nu un export manual. Auto-accept-ul inteligent se configurează tot pe canal și calculează ETA din KDS. Verificare: `list_entities(entityType: "delivery_channels", brandId)` — canalul există; statusul Online se vede în aplicație după conectare.
...
- **`create_delivery_channel` nu conectează nimic** — e doar înregistrarea canalului. Comenzile vin abia după credențiale + sincronizare meniu din aplicație. Nu raporta „Glovo conectat" pe baza apelului reușit; pentru Glovo, accept/gata/predare pot fi trimise prin Symbai, dar refuzul/anularea se fac în aplicația/suportul Glovo și se confirmă apoi automat înapoi.
...
- **Pornirea/oprirea agenților de vânzări** — verifică întâi catalogul conexiunii: dacă ai `toggle_sales_agent`, folosește-l; dacă nu apare, nu promite porniri/opriri prin conexiune — ghidează în aplicație (`list_sales_agents` rămâne mereu pentru citire).
