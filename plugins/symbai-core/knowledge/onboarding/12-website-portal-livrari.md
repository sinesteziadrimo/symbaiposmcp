...
Pentru Glovo, agentul trebuie sa explice pasii reali:
- se obtin de la Glovo Partner API token, Glovo Store ID si Store Address External ID;
- pagina Integrari livrare afiseaza cele trei URL-uri publice de trimis la Glovo: dispatch webhook, cancellation webhook si Menu JSON URL;
- full menu sync trimite la Glovo un `menuUrl` dupa validare, iar update-urile mici de pret/disponibilitate merg prin bulk update;
- auto-accept smart Glovo foloseste KDS-ul pentru ETA promis; daca ETA trece peste limita maxima, comanda ramane pentru operator;
- Symbai poate trimite accept/ready/pickup catre Glovo, dar nu promite refuz/anulare din Symbai: API-ul public Glovo cere anulare/refuz in Glovo/suport Glovo, apoi statusul vine inapoi prin webhook.
...
- **Credențialele platformelor de livrare + sincronizarea meniului** — `gaseste_in_aplicatie("conectare Glovo Wolt manager canale")` → tab Integrări (datele de la platformă), apoi tab Meniu & Prețuri (sincronizare). Pentru Glovo, copiaza catre Glovo dispatch webhook, cancellation webhook si Menu JSON URL afisate de Symbai; sincronizarea full menu foloseste `menuUrl`, nu un export manual. Auto-accept smart se configureaza tot pe canal si calculeaza ETA din KDS. Verificare: `list_entities(entityType: "delivery_channels", brandId)` — canalul există; statusul Online se vede în aplicație după conectare.
...
- **`create_delivery_channel` nu conectează nimic** — e doar înregistrarea canalului. Comenzile vin abia după credențiale + sincronizare meniu din aplicație. Nu raporta „Glovo conectat" pe baza apelului reușit; pentru Glovo, accept/ready/pickup pot fi trimise prin Symbai, dar refuzul/anularea se fac in Glovo/suport Glovo si se confirma inapoi prin webhook.
