...
- **Cheile/activarea platformelor de livrare** (după `create_delivery_channel`): `gaseste_in_aplicatie(intrebare="integrări canale livrare Glovo")`.
...
- **`create_delivery_channel` nu aduce comenzile.** Creează doar înregistrarea canalului; fără cheile API puse în aplicație, comenzile de pe platformă NU intră în POS. Spune explicit userului că urmează pasul din aplicație.
