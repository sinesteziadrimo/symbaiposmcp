...
- Ia segmentul de cale, nu tot URL-ul; scoate parametrii (`?...`), `.html`, ID-uri numerice pure.
- Dacă **NU** trimiți slug, platforma generează **automat** unul curat din nume (tot ASCII, RO-aware, unicitate garantată) — corect, dar nu identic cu URL-ul vechi. Așa că pentru un magazin existent care se mută pe Symbai, **preferă slug-ul sursă**; pentru produse complet noi, lasă auto-generarea.
- Slug-urile produselor sunt unice **global**; ale categoriilor — unice **per brand**. Platforma rezolvă singură coliziunile (adaugă `-2`, `-3`).

Verificare după import: deschide `/produs/<slug>` (sau preview `?page=/produs/<id>`) și confirmă că URL-ul are denumire lizibilă, nu doar `id=`. La categorii, `list_menu_categories` arată `slug`-ul fiecărei categorii.
