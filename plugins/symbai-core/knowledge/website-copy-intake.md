...
- **Meniu restaurant static:** pentru pagini de prezentare / clone cu meniu vizual (grid cu poze sau lista cu pret), foloseste `menu-list` (`layout`, `items[]`/`categories[]`, `price`, `meta`, `tags`, `spice`, culori). Nu-l prezenta ca meniu POS live; daca userul vrea catalog sincronizat, foloseste datele reale si componenta legata la catalog.
