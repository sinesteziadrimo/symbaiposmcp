...
| Editează o recepție FĂRĂ factură (din comandă de aprovizionare, recepție de angajat, document creat prin MCP) | `get_reception_edit_policy({id: ID_NIR})` → `replace_reception_lines({id: ID_NIR, idempotencyKey, lines: [...]})` cu lista COMPLETĂ a liniilor dorite (păstrează `id`-ul liniei vechi când o corectezi). Răspunsul `correctionMode` spune dacă NIR-ul a fost păstrat (`economic_revision`) sau înlocuit (`replacement`). Pentru NIR cu factură primești `RECEPTION_EDIT_USE_INVOICE_EDITOR` cu `invoiceId`: corectează factura. |
...
## Editare liberă: documente nepredate contabilității

Cât timp recepția este postată, iar factura și recepția ei nu au fost predate aplicației Accounting (firma nu are conexiune Accounting activă sau documentul nu a fost încă livrat), sistemul deschide **editarea liberă**: `get_reception_edit_policy({id: ID_NIR})` întoarce `mode: "free"`, iar `get_incoming_invoice_workflow_details` / detaliul facturii arată `freeEditMode: true` și `accountingSync` cu dovezile. În acest mod:

- orice câmp al liniilor se poate corecta cu `update_incoming_invoice_line` pe factura manuală, pe cea din fotografie/aviz și pe eFactura fără XML păstrat (cantitate, preț, total, TVA, descriere); valorile originale rămân în `original_*`, iar modificarea este auditată. Pe eFactura oficială cu XML ANAF păstrat, cantitatea, prețul, TVA și denumirea liniei sunt ale documentului oficial și determină totalurile: nu se rescriu (`EFACTURA_OFFICIAL_LINE_FISCAL_TUPLE_IMMUTABLE`), corectezi prin storno sau notă de credit de la furnizor; maparea produsului, gestiunea, conversia ambalajului și lotul rămân editabile;
- furnizorul, numărul, data, moneda și totalurile se corectează cu `update_incoming_invoice_context` pe factura manuală, pe cea din fotografie/aviz și pe eFactura fără XML păstrat; factura intră automat în corecție și se finalizează cu `correct_confirmed_reception`. Furnizorul se schimbă alegând `supplierId` (nu doar rescriind numele sau CUI-ul), iar unitatea/gestiunea recepției se schimbă într-o salvare separată de datele de identitate ale facturii. Pe eFactura oficială cu XML ANAF păstrat, furnizorul, numărul, data, moneda și totalurile vin din XML și nu se rescriu (`EFACTURA_OFFICIAL_HEADER_IMMUTABLE`): corectezi prin storno sau notă de credit de la furnizor, iar furnizorul în nomenclator; scadența, metoda de plată, observațiile și tratamentul fiscal (deductibilitate, cheltuială în avans) rămân editabile;
- `correct_confirmed_reception` acceptă și schimbări de produs, cantitate, gestiune sau furnizor, inclusiv pe factura generată din recepție: NIR-ul se înlocuiește automat (cel vechi rămâne anulat în istoric), stocul și notele contabile se refac, iar costurile dependente (FIFO, consumuri, transferuri) se recalculează în fundal;
- pe factura **generată din recepție**, încă nefinalizată, cantitatea și unitatea de măsură sunt ale recepției: corectarea lor cu `update_incoming_invoice_line` pregătește o corecție de recepție (`stagedForNirUpdate`), pe care o aplici cu `correct_confirmed_reception` trimițând numărătoarea verificată (`physicalQuantities` și `physicalVerificationConfirmed`). Factura revine la starea de dinainte (o factură generată nefinalizată rămâne nefinalizată), iar `finalize_received_invoice` stinge apoi datoria provizorie 408 pe recepția refăcută. Dacă era deja finalizată, redeschide-o întâi cu `retract_received_invoice` (în ecran: „Redeschide factura"), care stornează stingerea 408 și evaluarea; `reopen_accounted_bill_invoice` este pentru facturile de servicii fără NIR și refuză o factură cu recepție.
- NIR-ul fără factură se editează cu `replace_reception_lines`; recepția cu factură legată se corectează din factură.

Rămân protejate și în acest mod: id-ul de descărcare ANAF, XML-ul și documentul brut, identitatea eFacturii oficiale cu XML păstrat (furnizor, număr, dată, monedă, totaluri), unitatea/gestiunea din antet (se schimbă doar prin înlocuirea NIR-ului), legăturile documentului, luna închisă în POS, loturile cu proveniență ambiguă și producția dependentă din fabrică. Când `mode` este `sync_safe` (document predat sau în curs de predare către Accounting, ori provenit din Accounting), se aplică regulile obișnuite de mai sus, iar Accounting primește anularea și înlocuitorul sau corecția economică.

## Facturi de servicii deja contabilizate, fără NIR

Pentru o corecție cerută de utilizator, inclusiv împărțirea cheltuielii 50/50 între unități, citește mai întâi factura și alocările existente. Dacă editarea este refuzată deoarece factura este deja contabilizată fără NIR, descoperă `reopen_accounted_bill_invoice`: cu `invoiceId` și `confirm:false` verifică `canReopen`, nota și data propusă. Dacă previzualizarea permite operația și corecția este autorizată, folosește același tool cu `confirm:true`, aplică modificările și finalizează prin toolul disponibil de finalizare a facturii (`create_nir_from_invoice` recunoaște liniile exclusiv de cheltuială și finalizează fără NIR). Verifică separat noua înregistrare și distribuția salvată. Nu te opri la instrucțiunea de a apăsa un buton dacă ai aceste tooluri disponibile. Nu finaliza înainte să salvezi corecția și nu repeta redeschiderea după un rezultat necunoscut fără recitire.

Acest flux este pentru facturi **fără NIR**; nu îl aplica unei recepții de marfă pentru data sau prețul ei. Un refuz din previzualizare se explică după motivul exact. Nu inventa unitatea beneficiarului dintr-o altă factură încă neconfirmată; folosește destinația cerută sau dovezile verificate.

## Repartizarea cheltuielii și regulile memorate

Folosește repartizarea cerută pentru furnizorul sau facturile vizate. O alegere 50/50 pentru contabilitate ori pază nu autorizează aceeași repartizare pentru alți furnizori. Lipsa unei adrese sau același cod de afiliat nu dovedește procentele. Citește întâi nota facturii și indiciile de livrare; dacă nu stabilesc unitatea, păstrează decizia verificată sau cere numai informația lipsă.

`set_invoice_expense_allocations` cu `docAllocations:[]` elimină alegerea documentului și permite regulilor generale să se aplice; nu înseamnă „100% pe unitatea din antet” și nu șterge regula furnizorului. Pentru un document integral pe o unitate, trimite explicit acea unitate cu 100% și recitește alocările. Dacă documentul are alocări explicite, `get_invoice_expense_allocations` poate întoarce `supplierRule:null` fiindcă sugestia nu este evaluată. **Nu afirma că regula furnizorului a fost ștearsă sau corectată pe baza acestui null.** Alegerea documentului și regula pentru facturi viitoare se verifică separat; nu salva în memorie o concluzie neverificată despre regulă.
...
Pe NIR-urile importate, `sourceIncomingInvoiceLineId` poate lipsi, iar lotul și termenul pot exista numai pe NIR. Citește ambele documente înainte să interpretezi o mapare corectată ca schimbare de marfă. Dacă produsul corectat este chiar cel deja recepționat, păstrează loturile, cantitățile și gestiunile existente și aplică `correct_confirmed_reception` fără o nouă numărătoare. O potrivire unică și verificată permite păstrarea NIR-ului; verifică efectiv `correctionMode:economic_revision` și identitatea loturilor. Nu inventa legături între linii ambigue.

La un refuz de cantitate, compară pe linia indicată unitatea facturii, unitatea de stoc și conversia ambalajului: 6 recipiente × 7,5 kg pot reprezenta cele 45 kg deja recepționate. Nu trata automat diferența numerică drept surplus fizic și nu crea aviz înainte de verificare. Dacă versiunea live refuză o corecție de asociere deși marfa a rămas identică, raportează factura/NIR-ul și eroarea exactă; nu anula producția dependentă numai pentru a ocoli acel refuz. Blocarea de calitate se verifică separat și nu se ridică automat prin corectarea facturii.
...
În firmele clasificate ca fabrică, protecția privește **producția**, nu orice consum: dacă loturile recepției au fost doar **mutate între gestiuni** printr-un transfer, corecția fizică trece (lotul destinație își păstrează proveniența, iar diferența intră pe fluxul obișnuit de înlocuire). Rămâne refuzată (`INVENTORY_LOT_ALREADY_CONSUMED`) recepția ale cărei loturi au intrat în producție — direct sau după transfer — și cea consumată altfel decât prin transfer (vânzare, casare, ieșire). Refuzul enumeră loturile și operațiile vinovate: corectează întâi producția dependentă.
