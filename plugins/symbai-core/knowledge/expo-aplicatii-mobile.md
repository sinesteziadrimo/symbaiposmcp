...
- **Planul de sala pe telefon**: canvasul are pan/pinch, double tap, butoane flotante `+ / zoom / -` si reset safe-area. Butonul din mijloc arata zoomul relativ curent, nu text fix `1x`. Incadrarea tine cont si de mesele/decorurile care ies din dimensiunea de baza a canvasului, deci nu considera un plan "taiat" doar pentru ca `canvasWidth/canvasHeight` par mici. Pentru dovada vizuala foloseste emulator Android sau device real; arata screenshot cu Rooms -> planul salii, nu doar un build/typecheck.
