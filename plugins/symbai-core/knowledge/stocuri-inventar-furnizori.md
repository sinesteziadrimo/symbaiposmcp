...
Pentru modificarea sau anularea recepțiilor, citește [Corectarea recepțiilor prin MCP](corectare-receptii-mcp.md). Același ghid se aplică ChatGPT, Codex și Claude Code. Data intrării și prețul se corectează pe documentele existente fără storno inutil, inclusiv după consum; verifică separat FIFO prin `get_reception_cost_recalculation` și sincronizarea contabilă. Folosește acordul existent și păstrează cantitățile fizice dacă nu s-a cerut schimbarea lor.

> Pentru linkul exact către orice pagină folosește tool-ul `gaseste_in_aplicatie` — el e sursa autoritară de navigare.

## Pe scurt

Modulul acoperă tot drumul mărfii: intrarea (facturi de la furnizori → recepție/NIR), depozitarea (gestiuni, zone, loturi cu expirare), ieșirea (consum automat din vânzări, fișe de ieșire manuale, transferuri) și aprovizionarea (furnizori, cataloage, comenzi, recomandări de preț). Stocul se ține pe loturi. Alegerea fizică poate prioritiza termenul de valabilitate (FEFO), iar la termene egale intrarea mai veche. La restaurante, evaluarea financiară FIFO urmărește cronologia intrărilor și ieșirilor; o corecție de dată/preț actualizează costurile afectate în fundal, păstrând cantitățile și trasabilitatea fizică.
...
- **Lot / FEFO și evaluare FIFO** — loturile păstrează marfa fizică, expirarea și trasabilitatea. Prioritatea „expiră primul” privește alegerea fizică; la restaurante costul financiar FIFO folosește ordinea intrărilor și se poate actualiza după corectarea unei recepții vechi. Actualizarea costului nu mută fizic marfa și nu rescrie rețetele folosite.
  ⚠ **„Metoda de evaluare” din Setări → Stocuri (FIFO / LIFO / medie)** privește estimările afișate; nu mută loturile fizice și nu înlocuiește recalcularea financiară FIFO a restaurantului. Verifică funcțiile oferite de versiunea live conform [ghidului de corecții](corectare-receptii-mcp.md); nu extinde automat acest comportament la producția de fabrică.
...
Factura intră pe una din cele 4 căi (eFactura/ANAF, poze cu OCR, push din contabilitate, manual) → verifici/aprobi mapările liniilor (pe factură sau centralizat în `/inventory/ai-review`) → în `/stock-entries`, tab Recepții (NIR): „Recepție Nouă", alegi factura sursă și depozitul → postezi NIR-ul → marfa intră în stoc pe loturi și se generează notele contabile după verificarea și evaluarea facturii. Recepția din poză poate rămâne fizică, cu evaluarea în așteptare: verifică `get_reception_accounting_status` și finalizează factura verificată prin `finalize_received_invoice` când este necesar.
...
- `list_work_warehouses` — gestiunile de lucru permise contului nominal, cu brandurile și locațiile lor. Folosește-l înainte de recomandări, forecast sau necesar; rezolvă paginarea și alegerea brandului la o gestiune comună.
- `list_procurement_recommendations(warehouseId, brandId, productId?)` — compară furnizorii pentru aria aleasă. Fără cantitate cerută, totalurile sunt ilustrative, bazate pe `pricingAssumptionInternalQty`; compară prețurile pe aceeași unitate și monedă și explică ipoteza. `prepare_procurement_forecast(warehouseId, brandId, forecastDays, productId?)` calculează necesarul și cantitatea executabilă, fără comenzi. Explică sursele și blocajele din răspuns; nu confunda limita de analiză cu stoc sau necesar zero. Aplicarea autorizată folosește planul complet întors și `commit_procurement_forecast`; detalii în skill-ul `comanda-furnizor`.
...
- `reprocess_daily_consumption` 🔒 recalculează consumul pentru rețete/reguli corectate, ca job urmărit cu `get_reprocess_job_status`. Pentru o factură sau recepție greșită folosește corecția nominală din [ghid](corectare-receptii-mcp.md), care propagă diferențele de cost; nu începe cu vechiul `fix_reception_costs` și nu reprocesa automat consumul doar pentru un NIR corectat.
...
- **De ce diferă costul (COGS) de cel din rețetă?** Costul realizat urmărește consumul înregistrat și valoarea de intrare a mărfii. La restaurant, evaluarea financiară FIFO poate actualiza costul istoric după corectarea unei recepții; nu îl deduce numai din lotul fizic ales după expirare. Livrările finalizate descarcă loturi imediat. Pentru dată/preț verifică `get_reception_cost_recalculation`; reprocesarea intervalului este pentru rețete/reguli de consum corectate ori un diagnostic separat (vezi `consum-zilnic-cost-marfa.md`).
...
- **Pot folosi o ajustare minus cu cost 0 ca să scad doar cantitatea și să păstrez valoarea lotului?** Nu. O ajustare negativă scoate cantitate din loturile existente în ordinea normală de descărcare și reduce valoarea la costul real al acelor loturi. `unitCost: 0` pe ieșire nu înseamnă „păstrează valoarea" și nu reevaluează marfa rămasă. Dacă recepția a avut greșită cantitatea, ambalarea sau greutatea netă, corectează/stornează documentul-sursă prin fluxul controlat și verifică din nou cantitatea **și** valoarea; nu folosi ajustarea minus ca scurtătură de preț. Dacă numai costul unitar al unei recepții verificate este greșit, folosește previzualizarea și corecția nominală din [ghid](corectare-receptii-mcp.md), care propagă diferențele de cost. Nu reprocesa automat consumul doar pentru corectarea NIR-ului; verifică documentul, perioada contabilă și rezultatul operației. Nu executa o corecție fiscală doar dintr-un calcul presupus.
- **Pot băga marfa pe stoc cu o ajustare în plus, ca să merg mai repede?** Nu, și e greșeala care costă cel mai mult. Marfa primită de la un furnizor intră DOAR prin factură → recepție. O ajustare pozitivă adaugă cantitate fără furnizor, fără preț de achiziție real, fără factură și fără datorie către furnizor: stocul crește, dar contabilitatea nu află niciodată că ai cumpărat ceva. Pe deasupra, plusurile de stoc se scad din costul mărfii vândute — destule ajustări pozitive împing food cost-ul spre zero sau sub zero, iar P&L-ul devine nefolosibil. Când userul zice „pune-mi 20 kg pe stoc", „adaugă stoc la X", „am primit marfă" sau „crește-mi stocul", asta e o RECEPȚIE: cere-i factura sau avizul (vezi skill-ul `receptie-factura-furnizor`, Pasul 0). Ajustarea pozitivă e legitimă în exact două cazuri: rezultatul unei **numărători fizice** (sesiune de inventariere, unde plusul e o diferență constatată, nu o intrare) și **încărcarea soldului inițial** la pornirea firmei (vezi `deschidere-solduri-stoc-initial.md`). În rest, dacă simți nevoia unei ajustări pozitive, întreabă-te ce document lipsește — și cere-l.
...
- **De ce nu văd Recomandări Aprovizionare?** O unealtă indisponibilă sau un acces refuzat cere verificarea versiunii și ariei contului. Dacă analiza reușește, dar nu are oferte, verifică mapările dintre cataloage și produse în gestiunea/brandul ales. Zero oferte nu dovedește necesar zero; pragurile de stoc se verifică separat.
