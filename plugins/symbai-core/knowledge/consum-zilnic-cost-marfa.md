...
**Anularea unei note de consum introduse manual** (consum separat, import din alt sistem, consum de eveniment): `preview_inventory_document_cancellation` → `cancel_inventory_document` cu motiv. Citește `reversalDate` din previzualizare și spune-l exact. În versiunile care includ corecția din 25.09.2026, storno-ul stocului are data documentului original, iar ajustările de cost FIFO ulterioare se stornează la data lor (`laterCostAdjustmentReversalDates`); stocul la data documentului revine ca înainte. Dacă previzualizarea arată totuși data de azi, versiunea instalată este mai veche: spune asta, nu o prezenta ca limită permanentă. Pentru o lună închisă contabil trimite `reversalDate` dintr-o lună deschisă. Consumurile generate automat pe zile se refac prin reprocesare, nu prin anularea documentului.
