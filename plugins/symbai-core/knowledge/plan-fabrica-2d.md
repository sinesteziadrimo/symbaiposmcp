...
- **Live Fabrică pe plan:** în `/factory-floor-plan`, butonul **Live Fabrică** suprapune execuția curentă peste harta fizică: operații active/planificate azi pe echipamente, lot, produs, cantități, operatori și containere aflate la locație. Datele vin prin polling din `/api/factory-floor/live-execution`; este read-only și nu modifică layout-ul.
...
- „Ce rulează acum pe cuptor / unde e lotul / cine lucrează?" → deschide `/factory-floor-plan`, activează **Live Fabrică** și arată screenshot cu overlay-ul live; apoi verifică prin `get_production_dispatch` sau `exec_list_active_operations` dacă userul are nevoie de raport/audit, nu doar de dovadă vizuală.
