...
- **Versiunea pentru clienți noi** — la provizionarea unei instanțe POS noi, Hub-ul folosește ultima versiune publică înregistrată în Hub, nu se bazează pe tagul Docker flotant `latest`. Dacă în Hub a rămas pinuit un tag semver mai vechi decât ultima versiune publică, provisioning-ul îl ignoră și pornește clientul pe versiunea publică curentă; tagurile explicite non-semver (sha, `stable`, build intern) sunt respectate pentru că nu pot fi comparate sigur.
...
- **Cont nou, dar lipsesc funcții sau tool-uri recente.** Nu încerca să repari prin pași de onboarding. Verifică întâi versiunea din Setări → Tehnic → Symbai Hub și, pentru suport intern, jobul de provisioning din Hub: conturile noi ar trebui să pornească pe ultima versiune publică; un build vechi indică o problemă de deploy/provisioning sau un tag non-semver pinuit intenționat.
