...
**Operarea MCP urmează catalogul live al tenantului.** Citește `list_deals` → `get_deal` și `list_sales_quotes` → `get_sales_quote`; pentru scrierile nominale folosește instrumentele disponibile descrise mai jos. Configurarea CRM, echipele, calendarul stil Calendly, Symbai Meet, materialele și Academia au [ghid dedicat](crm-configurare-calendar-training.md). Începe cu `get_crm_configuration_guide` dacă este disponibil. Documentația nu dovedește că tenantul are deja aceste funcții; nu utiliza nume legacy în locul celor descoperite live.
