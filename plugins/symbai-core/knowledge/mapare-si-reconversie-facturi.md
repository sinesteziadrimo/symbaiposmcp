...
Pentru modificări sau anulări după NIR și operații nominale complete, urmează [Corectarea recepțiilor prin MCP](corectare-receptii-mcp.md).
...
**Cum corectezi un factor învățat greșit.** Reguli de Mapare (`/inventory/mapping-rules`) → regula furnizorului pentru acea descriere → editezi factorul și unitățile. ⚠ **Cât timp regula rămâne greșită, se reaplică la fiecare factură nouă** — nu are rost să remapezi linia la nesfârșit fără să repari regula. Pe lângă regulă, sistemul ține și o memorie separată a conversiilor de ambalaj (de ex. „1 borcan → 2,9 kg”): prin conexiune o vezi cu `list_pack_conversions` și o ștergi pe cea greșită cu `delete_pack_conversion`. Recepțiile deja postate nu se schimbă; corectează-le separat pe recepție.
...
**Operații disponibile prin MCP:** regulile se întrețin cu `update_mapping_rule` / `delete_mapping_rule`; liniile se împart/reunesc cu `split_invoice_line` / `undo_invoice_line_split`; costurile se distribuie cu `absorb_invoice_line` / `undo_invoice_line_absorption`; gestiunile cu `set_invoice_line_reception_warehouse`; produsul propus cu `accept_invoice_proposed_product`. `resync_invoice_mappings` reia potrivirile eligibile. Citește schema și ghidul corecțiilor; documentele provenite din Accounting își păstrează protecțiile de identitate.

## Întrebări frecvente și capcane

- **După recepție cantitatea e de 24 de ori mai mare (sau mult prea mică).** Factorul de pachet: fie s-a aplicat unul greșit, fie lipsește. Valoarea liniei rămâne mereu cea din factură — se schimbă doar cantitatea și prețul unitar. Verifici cu `get_received_efactura_details` (cantitatea și prețul originale ale furnizorului vs cele mapate și factorul aplicat) și în Reguli de Mapare. Dacă NIR-ul **nu** e făcut, remapezi cu factorul corect (numărul de bucăți din pachet) sau fără factor; dacă NIR-ul e postat, pregătești maparea prin `correct_invoice_line_mapping` și aplici `correct_confirmed_reception`. Cantitatea fizică se corectează numai cu numărătoare verificată și parametrii ei expliciți. **Obligatoriu corectează și regula învățată**, altfel se reaplică la următoarea factură.
...
- **Am o factură care nu se lasă modificată.** Facturile împinse din contabilitate au identitatea înghețată; se poate schimba doar conversia de ambalaj. Restul se corectează în contabilitate. După crearea NIR-ului, folosește operațiile nominale din ghidul corecțiilor; existența recepției nu înseamnă că lipsește operația MCP.
