...
- **Gestiuni & zone de depozitare** — ce e o gestiune, ce e o zonă, unde stă cantitatea, cum muți sau închizi o gestiune. Include regula din producție: fabrica = brand × locație, iar munca de pe tabletă se decide de ZONĂ, nu de gestiune. → `gestiuni-magazii-zone.md`
...
- **Acțiuni automate** — reguli «declanșator → efecte» pe toată platforma, cu accent pe fabrică: loturi & expirări, HACCP & temperaturi, utilaje & mentenanță, randament/rebut/cost, stoc minim → draft PO; rețete gata făcute, compoziții complexe (condiții pe efecte, follow-up întârziat, scări de escaladare, lanțuri) și administrare conversațională prin MCP. Ce nu se poate construi azi → tichet automat către echipa Symbai (`request_automation_capability`). → `actiuni-automate-fabrica.md` + skill `creeaza-automatizare`
