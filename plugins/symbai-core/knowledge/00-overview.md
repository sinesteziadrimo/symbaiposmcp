...
- **Acțiuni automate** — reguli «declanșator → efecte» pe toată platforma, cu accent pe fabrică: loturi & expirări, HACCP & temperaturi, utilaje & mentenanță, randament/rebut/cost, stoc minim → draft PO; rețete gata făcute și administrare conversațională prin MCP (`list_automation_triggers` → `create_automation_rule` → `test_automation_rule`). → `actiuni-automate-fabrica.md`
