...
- **Server & Fiscal** `/settings?tab=edge-server` — Titlu „PC & Server”. Adaugi PC-urile locației (primul devine server), carduri certificat tenant/wildcard, Failover Edge per locație (mod rețea DNS, Forțează conexiune, Print Agent pull/WS), listă device-uri cu „Fă Server”/test Print Agent/descarcă installer, Loguri Live, ghid instalare.
