...
- **Meniu & Dispozitive POS** (`/menu`) — Hub meniu cu 5 tab-uri reale: Prețuri Meniu, Platforme, Configurare Afișaj (POS/Web), Meniu Fizic, Oferte & Promoții.
  ...
  - Tab **Configurare Afișaj** (`/menu?tab=display`) — Profile de afișare meniu per canal (POS Ospătar, Kiosk Client, Website/Online, Platforma Clienți Web, Table—Clienți QR): teme, layout carduri, ordine/navigare categorii, imagini. Tot aici e **Politica de reduceri**: pragul de procent și de sumă sub care angajatul aplică reducerea fără aprobarea managerului (per platformă).
...
- **Setări** (`/settings`) — Pagină hub cu meniu lateral pe secțiuni (Companie, POS, Personal, Hardware, Stocuri, Marketing, Contabilitate, Asistent AI, Tehnic, Sistem); ?tab= deschide fiecare secțiune.
  ...
  - Tab **Server & Fiscal** (`/settings?tab=edge-server`) — Titlu „PC & Server”. Adaugi PC-urile locației (primul devine server), asociezi opțional un display KDS și/sau Workstation Tablet, carduri certificat tenant/wildcard, Failover Edge per locație, listă device-uri cu „Fă Server”/test Print Agent/descarcă installer, Loguri Live și ghid instalare. Installerul unic pune Print Agent + Edge + Mesh Control și creează scurtăturile asociate. _Notă: Pentru instalare ghidată: `/onboarding/print-agent`. Edge Server-ul oferă POS local funcțional fără internet._
