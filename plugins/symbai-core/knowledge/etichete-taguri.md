...
2. **Numărul REAL vine din dry-run, NU din `list_menu_categories`.** `list_menu_categories` întoarce `productCount` care numără **articole de meniu** — un produs aflat în 2 meniuri e numărat de 2 ori. Exemplu real (Drunken Rat): ramura BERE însuma 36 după category counts, dar `search_products_for_tagging` a găsit **31 produse distincte**. Pentru „câte produse primesc tagul" folosește dry-run-ul.
...
- **Tag NOU pentru rutare = bonuri pierdute** până când există regula tag→imprimantă/KDS. Un tag creat de tine NU rutează nicăieri singur — produsele lui generează bonuri „unrouted" care nu se printează și nu apar pe niciun ecran, **fără nicio eroare**. Regula de rutare NU se face prin MCP — se creează în aplicație (Setări → Imprimante / ecrane). După ce creezi un tag de secție nou, spune-i utilizatorului EXPLICIT ce tag și unde trebuie să iasă, și cere un bon de test.
- **`brandId` poate fi `null` pe taguri vechi.** `create_tag` setează corect `brandId`, dar taguri create prin alte căi (import, `auto_tag_from_menu_categories`) pot avea `brand_id = null` (caz real: tagul „BUCATARIE DRUNKEN"). `list_tags(brandId)` le include oricum (filtrul nu exclude null-urile), deci nu te speria — dar dacă ai nevoie de izolare strictă pe brand, verifică `brand_id` în răspuns.
...
- **Produsele sunt globale pe firmă** — la clienți multi-locație, izolarea se face la regulile de rutare (per locație, în aplicație), nu la etichete. O etichetă „de barul X" pe produse vândute și în localul Y poate ruta bonuri la alt local dacă regula nu e limitată pe locație (incident real în producție).
...
## Tabele SQL (cu token SQL read-only)

- `tags` — `id, name, brand_id, color, entity_types (text[]), description, created_at`.
- `tag_assignments` — `id, tag_id, entity_type, entity_id, sort_order, created_at`.
- Reguli de rutare (citire, dacă există): `tag_routing_rules` / `printer_tag_routes` — `SELECT t.name, r.printer_id, r.screen_ids FROM tag_routing_rules r JOIN tags t ON t.id=r.tag_id WHERE r.active`.
