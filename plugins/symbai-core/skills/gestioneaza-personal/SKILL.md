...
**Tot ce e scriere cere modulul `personal` pe token și permisiunile nominale ale utilizatorului.** Context mereu întâi: `list_brands` + `list_locations` (ai nevoie de brandId/locationId). Roluri: `list_entities(entityType:"roles", brandId)`. Stare curentă pe brand/unitate: `get_staff_overview(brandId, locationId)`. Citește schema live înainte de apel: `list_employees` poate fi o citire de operatori pentru o gestiune, nu inventarul complet al personalului.
...
ID-uri, nu nume (roleId, employeeId, floorConfigId). Caută înainte de a crea. După scriere verifică prin CITIRE. Pentru modificarea unei programări folosește `update_staff_schedule(scheduleId, ...)`. `delete_staff_schedule` permite ștergerea unei programări: citește întâi previzualizarea și folosește `confirm:true` în limitele acordului utilizatorului. Nu șterge programări legate de pontaje reale ca să ascunzi un conflict și nu confunda ștergerea programului cu ștergerea prezenței.

**Citire completă înainte de programare:** cere perioada exactă, personalul, programul existent, alocările de producție și concediile aprobate. La `get_staff_overview`, folosește `dateFrom`, `dateTo`, `query`/`employeeId`, `active` și `collection` dacă apar în schema live. Urmează `pagination.nextArguments` până la `hasMore:false`; prima pagină nu reprezintă toți angajații. Pentru un singur tip de detalii alege colecția respectivă. Dacă versiunea live oferă numai `includeDetails`, `limit` și `offset`, citește pagini mici și verifică totalurile și perioada efectiv întoarsă. Un răspuns `complete:false`/`dataOmitted` sau un interval mai scurt decât cel cerut nu dovedește că ai toate datele. Nu construi o programare completă dintr-un eșantion și nu transforma programul planificat în pontaj efectiv.

**Lucru eficient după citirea inițială:** grupează toate câmpurile cerute pentru aceeași persoană într-un singur `update_employee`. Pentru verificare colectivă folosește `get_staff_overview(collection:"employees", employeeIds:[...])` dacă schema live oferă lista. Păstrează ID-urile, perioada și rezultatele verificate. La recitirea aceleiași pagini poți trimite `ifNoneMatch` cu `revision` primit; `notModified:true` înseamnă că pagina a fost verificată și datele precedente pot fi refolosite. Nu trimite revizia la altă pagină și nu presupune că programul sau pontajul au rămas neschimbate doar pentru că fișa angajatului este identică.

**Operatori în fabrică:** pentru alocări pe ture/utilaje/zone folosește `bulk_assign_operators_to_shifts` dacă apare în catalogul live. Trimite gestiunea verificată și `assignments` cu câte un `employeeId`, `shiftId`, lista `dates` și stația efectiv cerută. Maximum 1000 de alocări pe apel, inclusiv repetările pe zile. `preview:true` verifică întregul lot; la aplicare păstrează `expectedRevision` din previzualizare și acordul deja dat. Omiterea zonei/utilajului/rolului păstrează valoarea existentă; `null` o elimină explicit. Rezultatul colectiv include ID-urile și stațiile salvate, plus avertismentele de specializare. Verifică prin `list_shift_assignments` cu `compact:true`, intervalul sau `assignmentIds`, urmând `pagination.nextArguments` până la final, fără sute de apeluri individuale. Tabelele compacte păstrează câmpurile în `columns` și valorile fiecărei înregistrări în `rows`. `bulk_create_shift_assignments` simplu nu fixează utilaje și nu înlocuiește acest flux. Nu transforma alocarea de producție în pontaj real.

## (a) Adaugă angajat + parolă/PIN

1. `create_employee(firstName*, lastName*, brandId*, roleId*, email, phone, locationId, hireDate, baseSalary, nickname, pin)`. Alege un **rol activ** din catalog; email-ul nu este obligatoriu. Citește schema live pentru câmpurile disponibile de salariu și unități. Poreclă = numele afișat peste tot.
   - Confirmă fișa și unitățile după creare. Un angajat fără brand este disponibil pe toate brandurile; fără locație, pe toate locațiile. Cele două axe sunt independente, iar permisiunile rolului rămân aplicabile. Nu atribui artificial un brand sau o locație doar ca să apară în listă. Dacă lipsește dintr-o citire, verifică filtrul, paginarea, starea activă și aria utilizatorului înainte să schimbi fișa.
...
0. **ÎNTÂI verifici dacă rolul cerut există deja: `find_role_for_job(job)`** (rol real ▸ rol prestabilit ▸ abia apoi custom). La o locație nouă, `plan_location_roles(brandId)` îți spune ce roluri îi lipsesc. Nu porni niciodată direct cu `create_role`.
1. Rapid, din prestabilite: `apply_role_presets(roleNames[]*, brandId?, confirm:true)` — creează exact rolurile cerute cu permisiunile gata setate, fără duplicate. Pentru TOT setul unei verticale: `seed_default_roles(brandId*, businessType*)` (restaurant|bar|cafenea|fast_food|hotel_restaurant|catering…), idempotent.
2. Rol chiar nou (nu există ca prestabilit): `create_role(name*, brandId*)`. **⚠ NU trimite `permissions` ca OBIECT la `create_role`** (ex. `{pos:true}`) — se salvează greșit (rolul rămâne fără permisiuni reale) ȘI strică apoi apelurile incrementale `set_role_permissions(addPermissions)` (dau eroare). **Creează rolul gol, apoi pune permisiunile cu `set_role_permissions` (array).**
...
2. **⚠ DOUĂ evidențe de tură separate — alege corect:**
   ...
   - `create_staff_schedule(employeeId*, scheduledStart*, scheduledEnd*, brandId, locationId, floorConfigId, sectionId, sectionName, status)` creează programarea din **Planificator Ture**, inclusiv raionul. Trimite unitatea și raionul cerute; brandul omis se poate deduce din angajat.
   - Pentru programul în planificator folosește `create_staff_schedule` sau `bulk_create_staff_schedules`, cu `status:"published"` când utilizatorul a cerut publicarea. Modifică programarea prin `update_staff_schedule(scheduleId, sectionId, sectionName, ...)`, fără ștergere și recreare. Numele raionului trebuie să corespundă exact celui de pe mese. Pentru tura POS folosește separat `create_shift`/`update_shift` și `bulk_create_shifts`.
   - **Ore locale:** pentru programările MCP trimite ora României cerută, de exemplu `2026-09-14T10:00`, conform schemei live. Nu scădea manual diferența de fus orar și nu adăuga `Z` unei ore locale. Un timestamp cu offset explicit trebuie să reprezinte același moment (de exemplu `2026-09-14T10:00:00+03:00`). Pentru o tură de noapte, sfârșitul programării este în ziua următoare. Recitește rezultatul și verifică afișarea în `Europe/Bucharest`, inclusiv la schimbarea orei.
