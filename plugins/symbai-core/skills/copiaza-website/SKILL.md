...
- **`clone_category_tree_audit(jobId, brandId)`** — audit advisory pentru ierarhia categoriilor: detectează catalogul aplatizat și relațiile părinte-copil lipsă.
- **`clone_branding_audit(jobId, brandId)`** — audit advisory pentru logo/culoare/nume/social/contact, ca site-ul nou să arate ca brandul sursă.
...
- **`clone_redirect_map(jobId, brandId, apply?)`** — continuitate SEO: dry-run, apoi `apply:true` scrie 301-uri din URL-urile vechi produs/categorie spre URL-urile Symbai. Rulează separat după PASS.

## ⚠ Reguli NON-NEGOCIABILE (din ele vine corectitudinea)
1. **„Gata" NU înseamnă „pare copiat".** „Gata" = `clone_audit_all(...).data.pass === true` sau, dacă rulezi manual, TOATE porțile dure `pass:true` — `clone_parity_diff` (0 produse lipsă, numitor sigur) ȘI `clone_fidelity_audit` (câmpurile produselor transferate, inclusiv galerie când sursa are 2+ poze) ȘI `clone_coverage_audit` (categorii/blog/pagini legale migrate) — PLUS `audit_shop_health` fără `error`. Niciodată pe baza impresiei tale.
...
10. Buclă până: coada goală (0 pending în `list_clone_crawl_pages`) ȘI `clone_audit_all(...).data.pass === true` (sau cele 3 porți dure `pass:true` + advisory rezolvat/N-A) ȘI `audit_shop_health` fără `error`.
