...
- Pentru **paginile de prezentare** (non-produs): conținutul se scrie cu uneltele din `construieste-website` (`set_website_theme` → `update_website_navigation` → `set_website_page_content`/`set_website_legal_page`) și se verifică prin `get_website_page` înainte de browser — vezi Faza 4b.
