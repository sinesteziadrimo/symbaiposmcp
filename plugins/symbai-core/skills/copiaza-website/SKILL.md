...
3. **Cheia de identitate = SKU / handle / slug-sursă, NU numele.** Setează câmpul `sku` al fiecărui produs = cheia sursă la import (`bulk_create_products(sku=<cheie>)`). Altfel `bulk_create_products` deduplică pe NUME și îmbină tăcut produse distincte → pierdere de date care „trece" la numărătoare.
...
1. `list_brands` → `brandId` (sau creează brand+locație+depozit dacă instanța clientului e goală / cont nou — vezi `construieste-website`).
...
7. Buclă: `list_clone_crawl_pages(jobId, type:"product", status:"cached", limit, offset)` → ia un lot de ~25. Dă-l unui **sub-agent** (Task) cu instrucțiune strictă: pentru fiecare URL → `get_cached_page` → `bulk_create_products(sku=cheia sursă, name, description, vat:21, ...)` + `set_product_image/bulk_set_product_images` + `add_menu_item(productBrand, ...)`; întoarce DOAR `{lot, produseScrise, pozeScrise, eșecuri[]}`. Apoi categorii (ierarhie din breadcrumb → `bulk_reparent_menu_categories` + `set_products_menu_category`), blog (`bulk_create_blog_posts`), legale (`set_website_legal_page`), footer/branding.
   - ⚠ **Parcurge TOATE tipurile, nu doar `type:"product"`/`type:"page"`.** Coada are `product/category/blog/legal/page` — fiecare cu importatorul lui. Dacă filtrezi importul de pagini pe `type:"page"`, pierzi tăcut paginile cu `type:"legal"`/`type:"blog"` (într-o clonare reală au lipsit mai multe pagini exact așa). Iterează pe toate tipurile relevante.
...
3. **Pas vizual în browser** (Chrome): deschide URL-ul REAL și echivalentul local unul lângă altul și compară componentă-cu-componentă (poze, benzi, box offset, fonturi, CTA, animație reveal). Notează diferențele → repară maparea → re-importă → re-verifică. Pe pagini cu multe imagini screenshot-ul poate da timeout: verifică structura cu interogări DOM (componente prezente + datele lor), nu doar poze.
   - **Verifică EXPLICIT că fiecare rută randează conținutul EI** (acasă ≠ meniu; `/blog` = listarea blogului; `/<pagină>` = pagina ei, nu homepage-ul). Un **fallback tăcut la home/meniu** (pagină neimportată) = pagină LIPSĂ, nu PASS. Într-un caz real, homepage-ul arăta meniul și `/blog` arăta tot meniul exact din cauza asta.
...
**Capcane de mapare → Verificare** (fiecare a stricat o clonă reală). Bifează-le UNA CÂTE UNA pe fiecare componentă — nu „dintr-o privire":
...
- [ ] **`text-image` bogat, nu custom inutil.** Pentru secțiuni split folosește componenta nativă cu `html`, `eyebrow`, `eyebrowColor`, `titleColor`, `ctaColor`, `backgroundImageUrl`, `backgroundOverlay`, `imageBoxColor`/`imageBoxImage`, `imageMask`, `imageShadow`, `decor`. Abia dacă schema nu exprimă designul treci la `custom-html`.
...
- Hero index editorial: `index-hero` cu `items:[{label,image,url}]`, `height`, `align`, `overlayColor`, `activeColor`, `ctaText`, `ctaUrl` pentru liste verticale de locatii/destinatii/colectii care schimba imaginea pe hover/focus (hotel groups / The Standard). Nu il inlocui cu grila sau custom HTML.
...
- Beneficii in coloane minimaliste: `feature-cards` cu `style:"divided"`, `cards[].description` si `cards[].linkText`; pentru titlu de sectiune aliniat stanga/dreapta sau pe doua randuri foloseste `titleAlign` si `\n` in `title`.
- Carduri filtrabile: `tabbed-cards` cu `tabs[]`, `cards[].tab` si `showAllTab` pentru servicii sau portofolii filtrate. Pentru pagina reala de locatii/restaurante/hoteluri cu harta si directii, foloseste `store-locator`.
- Localizator cu harta: `store-locator` cu `locations:[{name,address,city,region,hours,phone,mapQuery,directionsUrl}]`, `regions`, `allRegionLabel`, `mapHeight` pentru pagini "Find us" / "Restaurants" / "Locations"; filtrele pe regiune sunt interactive si recentreaza harta.
...
- Meniu restaurant static: `menu-list` cu `layout:"grid"|"list"`, `items[]`/`categories[]`, pret, meta/kcal, badge-uri, tag-uri dietetice, spice, `filters:true`, `filterTags` si culori. Foloseste-l pentru pagini de marketing/clone, inclusiv filtre Vegan/Vegetarian/Gluten-free; daca userul cere meniu POS live, nu-l folosi ca substitut al catalogului.
...
- Navbar/chrome: `update_website_navigation` aplica `navbarSettings` direct, inclusiv `sidebar-left`, `navbarBg` pentru bare solide pe fundal cream/branded, `logoRotate:180` pentru wordmark rasturnat, `logoBg` pentru cutie colorata in spatele logoText si `ctaButton:{style:"outline"}` / `ctaButton:{style:"outline",color:"#c9a45c"}` pentru CTA contur cu culoare default sau custom. Daca `navbarBg` e inchis la culoare, rendererul comuta automat textul/logo/iconitele pe light; verifica prin `get_website_page` + screenshot dupa scriere.
...
- Navbar transparent / logo contextual: `navbarSettings.transparent:true` pluteste peste hero doar daca pagina incepe cu `hero`, `hero-slider`, `image-hero` sau `index-hero`; pe pagini light devine solid si poate folosi `logoUrlDark`. Pentru selector valuta seteaza `showCurrency:true` + `currencies`. Verifica vizual home + cel putin o subpagina.
...
- Text+imagine cu muchie diagonala: pentru sectiuni split cu fotografie taiata oblic spre text, foloseste `text-image.imageMask:"diagonal"`; masca se intoarce automat cu pozitia imaginii, deci nu marca sectiunea ca fiind custom doar pentru aceasta forma.
