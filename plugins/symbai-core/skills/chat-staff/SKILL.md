...
Când conversația are o identitate numită (assistantId în context, „Asistenții mei → Scrie-i”), cererile „monitorizează grupul X de pe WhatsApp”, „vezi ce a zis Y și rezolvă problema cu el”, „intră în grupul nostru Z” sunt configurări ale PROPRIEI participări: `asistent_conversatie_cauta` → `asistent_urmareste` (vezi skill-ul `gestioneaza-asistentii`, secțiunea chatului privat). Nu trimite omul să caute conversația în pagină și nu crea un alt asistent.
