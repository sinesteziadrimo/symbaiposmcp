...
7. **Inventarierea se limitează strict la gestiunile alese** — când ajuți cu `/inventory-check`, lista de produse trebuie să vină din stocul live al gestiunii alese sau din produse stocabile cu zonă asignată acelei gestiuni. Nu ghida clientul să numere produse nestocabile, servicii sau produse finite de rețetă în pagina de depozitare.
8. **Magazii → zone → echipamente → poziții** — pagina dedicată este `/storage-designer`, pentru restaurant, depozit și fabrică. Rafturile există inclusiv cu stoc global. Citește `knowledge/organizare-depozitare-ai.md` și descoperă uneltele disponibile înainte să recomanzi sau să configurezi.
...
**H. Magazii, zone, rafturi, plan și etichete QR**
1. Citește `knowledge/organizare-depozitare-ai.md`. Verifică domeniul și fluxul real cu `get_business_context`/`get_location_context`, apoi `list_storage_warehouses` → `get_storage_workspace` (toate paginile). Folosește `get_storage_place_contents` pentru produse și containere, `list_lots` pentru expirare. Dacă familia nouă nu apare, caut-o cu `cauta_tool`; nu promite operații care încă nu sunt disponibile pe instanță.
2. Explică ierarhia: magazia ține evidența, zona delimitează spațiul, raftul/frigiderul este echipamentul, poziția este adresa scanabilă. Global = recomandări de amplasare fără cantități locale; zonal/poziții = solduri la precizia înregistrată. Containerele nu se adună peste aceleași produse din stoc.
3. Creează doar ce lipsește: `create_storage_warehouse`, `create_storage_place`, `create_storage_rack`; modifică prin `configure_storage_warehouse`/`update_storage_place`. Păstrează literele, zerourile, ordinea și forma adreselor clientului. Nu porni urmărirea zonală pentru a crea rafturi. Nu activa revenirea la global fără autorizarea ștergerii atribuirii zonale istorice.
4. La migrare folosește `preview_storage_import` → `import_storage_positions`, cu aceeași sursă stabilă și numele gestiunii DIN EXPORT (`warehouseName`). Maparea câmpului `rack` indică coloana Modul; păstrează codul Locație și id-ul extern ca text. Verifică/adoptă locurile existente fără a dubla stocul sau schimba QR-urile.
5. Cere dimensiunile și obstacolele care lipsesc; o schiță orientativă nu este un plan măsurat. Configurează dimensiunile, previzualizează cu `preview_storage_layout`, aplică exact planul valid cu `apply_storage_layout`, apoi recitește. Coordonatele sunt în cm față de părinte. Amplasarea desenului nu mută marfă.
6. Pentru etichete: `list_storage_label_printers` → `prepare_storage_labels` → `print_storage_labels` numai când tipărirea este cerută. Verifică imprimanta, locurile și copiile. Păstrează aceeași `idempotencyKey` și aceleași argumente la reîncercare; verifică prin `get_storage_label_job`. Confirmarea imprimantei nu dovedește că întregul lot a ieșit fizic. Fără imprimantă compatibilă, oferă etichetele A4 din pagină.
7. QR-ul conține identificatorul stabil al locului, nu un URL inventat și nici codul produsului. Arată rezultatul în `/storage-designer?warehouseId=…`. Pentru contextul întregii hale folosește separat skill-ul `plan-fabrica`; nu crea copii decorative ale rafturilor în locul locurilor reale.
...
6. Dacă clientul vrea doar să mute produsele **între zone ale aceleiași gestiuni**, folosește transferul între locuri din pagina de depozitare, cu urmărirea zonală activă; mută cantitățile locale, păstrând totalul și valoarea gestiunii. Simpla mutare pe desen nu înregistrează transferul. Vezi `knowledge/gestiuni-magazii-zone.md`.

## Inventare din urmă și produse unificate

Verifică disponibilitatea parametrilor și uneltelor pe instanța conectată cu `cauta_tool` și `citeste_tool`. Pentru liste primite din Excel, confirmă data, gestiunea, unitatea și cantitatea fizică. O celulă goală, o estimare sau soldul scriptic copiat în foaie nu reprezintă automat o numărătoare confirmată. Nu importa costuri estimate ca prețuri de intrare.

Rulează `preview_physical_inventory` cu `countDate` și lista confirmată; pentru o listă parțială folosește `unlisted:'keep'`. Diferența istorică se compară cu soldul la data numărării; soldul curent este o informație separată. Introdu listele în ordine cronologică. Dacă aceeași marfă are coduri unificate, folosește `includeMergedProducts:true` numai după confirmarea că numărătoarea include toate codurile respective. Nu introduce separat zero pe vechiul cod și totalul pe cel nou: numărătoarea trebuie să observe o singură cantitate fizică.

Aplică prin `apply_physical_inventory` în autorizarea primită și păstrează ID-ul sesiunii. La eșec citește răspunsul: o sesiune deschisă poate conține deja numărătorile; continuă sesiunea existentă prin fluxul oferit, fără inventar duplicat. După finalizare, `verify_physical_inventory(sessionId,offset,limit)` arată diferențele și verificările cantitative, costurile necunoscute, dezechilibrele contabile și lucrările în așteptare. Citește toate paginile până la `complete:true`. `financialVerificationComplete:false` înseamnă că acest raport nu certifică evaluarea financiară finală, chiar dacă `quantityAdjustmentsMatch` și `journalBalanceMatches` sunt adevărate. Verifică separat recalcularea FIFO și stocul curent cu `get_stock_levels`.

Dacă instanța oferă paginare pentru previzualizare, păstrează aceeași listă de numărări și citește toate paginile cu `offset` și `limit`, urmând `pagination.nextOffset`. Totalurile privesc lista completă, chiar dacă pagina conține doar o parte dintre diferențe. La verificare, liniile inventarului și lucrările în așteptare au paginări independente: urmărește și `pendingWorkPagination.nextArguments`, cu `pendingOffset` și `pendingLimit`. O pagină goală din una nu dovedește că cealaltă listă este completă.

Finalizarea poate continua după răspunsul inițial. La `status:'finalizing'` sau `outcomeUnknown:true`, păstrează `sessionId` și verifică aceeași sesiune cu unealta indicată în `nextTool`/`nextArguments`. Nu relansa aplicarea și nu crea alt inventar cât rezultatul este incert. Nici expirarea conexiunii nu dovedește anularea tranzacției. Confirmă separat finalizarea, documentele și diferențele; un refuz contabil cu `failure.code` și `failure.details` cere investigarea dovezilor respective, fără dezactivarea protecției.

Nu porni o a doua reprocesare peste una activă. Dacă vânzările trebuie refăcute după corectarea rețetelor ori a inventarelor, identifică un singur job și urmărește-l. Raportează separat cantitățile numărate, diferențele istorice, soldul curent și valorile încă provizorii. Nu compensa o lipsă de date prin recepții, transferuri ori prețuri presupuse.

## Tool-uri folosite
- **Organizare, plan și QR:** cele 16 unelte din fluxul H și `knowledge/organizare-depozitare-ai.md`; cer acces nominal și aria acordată. Capabilitățile asistentului pentru configurare și print se acordă separat.
