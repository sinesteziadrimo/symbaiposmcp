...
**Pas 6 — Magazii, rafturi și QR (când clientul întreabă „ce e unde?”).** Folosește skill-ul `gestioneaza-stocuri`, fluxul H, și `knowledge/organizare-depozitare-ai.md`. Pagina dedicată `/storage-designer` funcționează pentru toate domeniile. `get_storage_workspace` citește ierarhia și geometria, `get_storage_place_contents` citește produsele/containerele. `create_storage_rack` configurează adrese reale; `preview_storage_layout` → `apply_storage_layout` amplasează în interiorul magaziei. Pentru etichete: pregătire → tipărire cerută → verificarea comenzii, cu aceeași cheie la retry. Planul halei rămâne contextul exterior și poate referi aceste entități reale; nu crea rafturi doar ca obiecte decorative. Nu presupune că amplasarea din cele două editoare se sincronizează automat. QR-ul păstrează identificatorul locului; scanarea în Symbai arată conținutul și precizia reală a evidenței.
