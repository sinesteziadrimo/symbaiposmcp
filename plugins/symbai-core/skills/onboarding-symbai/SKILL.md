...
- **Tool-urile din sesiunea ta sunt sursa de adevăr** — dacă un tool din ghid lipsește, instanța nu are încă versiunea aceea; folosește alternativa UI din fișierul fazei. Dacă este un cont abia creat și lipsesc funcții recente, verifică versiunea în Setări → Tehnic → Symbai Hub sau cere suportului să verifice jobul de provisioning: Hub-ul pornește clienții noi pe ultima versiune publică, deci un build vechi este problemă de deploy/provisioning, nu pas de onboarding.
