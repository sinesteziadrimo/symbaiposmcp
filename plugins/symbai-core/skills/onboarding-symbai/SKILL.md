...
- **Import de date = avantajul tău unic**: ai trei căi — (A) creezi direct prin tool-uri din fișierul local (fișiere mici, curate); (C) **import asistat** — lași pagina de import să parseze (motor robust), TU răspunzi la întrebările ei prin extensia Chrome și corectezi după prin conexiune (fișiere mari/murdare/multe — calea preferată în practică); (B) userul face singur în wizard. Alegerea + pașii: `02-import-date.md`; playbook-ul căii asistate: `02b-import-asistat.md` (sau skill-ul `importa-date`). La retail/magazin, păstrează în import `sku`, `barcode` și `ean` pe produs, altfel scanarea la inventar/POS nu va funcționa după migrare.
