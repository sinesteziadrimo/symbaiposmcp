...
Ghidul complet e în pluginul curent, folderul `knowledge/onboarding/`:
...
- **`harta-pasi-wizard.md` — citește-l când userul e DEJA în wizard** (`/onboarding/step/N`) și te întreabă ceva la un anume pas. Index pe toți pașii wizardului: ce e fiecare, ce faci direct prin MCP, ce e UI-only, frază de help. Te orientezi instant și-l ajuți pe loc, fără să-l scoți din pas.
...
- **MCP-direct e calea preferată.** De cele mai multe ori e mai simplu și mai rapid să faci direct prin conexiune (tool-uri MCP) decât prin pagină/extensia Chrome. Pentru import Excel/CSV mic folosește întâi `create_bulk_import_session_from_file` → întrebări → `import_bulk_session`; pagina/extensia rămân pentru fișiere peste limită, PDF/poze, instalarea PC-ului, designere vizuale (sală, website, meniu fizic), OAuth (Viva/Meta/ANAF), generare/print QR. În rest: faci tu prin MCP → confirmi prin citire → „gata, ți-am făcut X, dă refresh".
...
- **Import de date = avantajul tău unic**: ai patru căi — (M) import fișier prin MCP pentru Excel/CSV mic; (A) creezi direct prin tool-uri din fișierul local (liste mici, curate); (C) **import asistat** — lași pagina de import să parseze și TU răspunzi la întrebări prin extensia Chrome (fișiere mari/murdare/multe); (B) userul face singur în wizard. Alegerea + pașii: `02-import-date.md`; playbook-ul căii asistate: `02b-import-asistat.md` (sau skill-ul `importa-date`). La retail/magazin, păstrează în import `sku`, `barcode` și `ean` pe produs, altfel scanarea la inventar/POS nu va funcționa după migrare.
...
- **Tool-urile din sesiunea ta sunt sursa de adevăr** — dacă un tool din ghid lipsește, instanța nu are încă versiunea aceea; folosește alternativa UI din fișierul fazei. Dacă este un cont abia creat și lipsesc funcții recente, verifică versiunea în Setări → Tehnic → Symbai Hub sau cere echipei de suport Symbai să verifice instanța: conturile noi primesc automat ultima versiune publică, deci lipsa funcțiilor recente pe un cont nou e o problemă de semnalat suportului, nu un pas de onboarding.
