...
3. **Lista live de tool-uri câștigă.** Conexiunea nominală citește pipeline-ul și ofertele (`list_deals` → `get_deal`; `list_sales_quotes` → `get_sales_quote`; sumar cu `get_sales_quote_summary`). Scrierile pe oportunități trec prin ACELAȘI serviciu tranzacțional ca interfața și cer contul nominal al angajatului (nu un token de organizație): `create_deal` (brandId exact, `operationId` UUID păstrat la retry — răspuns incert = repeți cu aceleași argumente, nu creează dublu; client existent `customerId` SAU `newCustomer` dedublat pe telefon/email; `value` se salvează DOAR împreună cu `currency` ISO; responsabilul implicit e persoana conectată, doar un coordonator poate alege un coleg), `advance_deal` (brandId + dealId + `operationId`; etapă, status open/won/lost, responsabil, valoare + monedă, date, avans, contract; valoarea e refuzată cu 409 când oferta principală o controlează), `log_deal_activity` / `mark_deal_activity_done` (activități, cu `source` pentru mesajul din email/WhatsApp), `create_sales_quote_draft` (ciornă de ofertă). Configurarea are instrumente nominale dedicate: `list_crm_pipelines`, `list_crm_pipeline_stages`, `create_crm_pipeline_stage`, `update_crm_pipeline_stage`, `list_crm_event_types` și salvările descrise în ghidul de configurare, când apar în catalogul live. Dacă un tool lipsește din lista live, spune asta și trimite la pagină, nu inventa.
4. **⚠ Două limite reale:**
...
5. **⚠ „Ofertă" = trei lucruri.** Oferta comercială pe deal are variante și o singură ofertă principală; ea conduce valoarea și moneda oportunității. Este distinctă de devizul tehnic și de `create_offer`, care e **promoție/discount pe bonul de restaurant**. La „fă o ofertă pentru clientul X", `create_offer` e tool-ul GREȘIT.
...
Preferă instrumentele din [ghidul de configurare](../../knowledge/crm-configurare-calendar-training.md): citire → modificare cerută → recitire. Nu reaplica wizardul peste un CRM existent doar pentru a schimba stilul sau o etapă. Interfața de mai jos rămâne alternativa pentru opțiunile fără instrument disponibil.
...
### Ofertele comerciale — ce contează zilnic

1. Un deal poate avea mai multe variante, dar **una singură este principală**. Numai aceasta conduce valoarea, moneda, split-urile și forecastul; variantele secundare nu se adună încă o dată.
2. Citește întâi `list_sales_quotes(dealId)`; pentru linii, note și termeni folosește `get_sales_quote(quoteId)`. Pentru portofoliu/manager folosește `get_sales_quote_summary(brandId)` — valorile sunt separate pe monedă și nu dublează variantele.
3. Statusurile sunt `draft`, `sent`, `accepted`, `declined`, `expired`, `converted`. O ofertă `sent` cu valabilitatea depășită este raportată efectiv `expired` chiar înainte de reconcilierea de fundal.
4. O ofertă trimisă este document comercial înghețat: modificările de preț/termeni se fac într-o variantă nouă. PDF-ul istoric se regenerează din snapshotul de la emitere, nu din datele clientului schimbate ulterior.
5. Tool-urile de ofertă de mai sus sunt **citire**. Crearea/editarea/trimiterea/acceptarea și alegerea ofertei principale se fac în fișa deal-ului / `/services/quotes`. O ofertă marcată `technical` se gestionează în workspace-ul tehnic, nu în fluxul standard.
...
- **Din fișa de Deal/Eveniment** — ce e UI-only: Produse (meniul evenimentului), Personal alocat, Prep/Producție, Cheltuieli, payment link, BEO, plus câmpurile de petrecere (tort, mascotă, decorațiuni, defalcare adulți/copii, chestionare/NPS, cronologie).
  Etapa pipeline, Won/Lost, valoarea + moneda, responsabilul, avansul plătit și contractul semnat se schimbă cu `advance_deal` (același serviciu ca UI-ul; păstrează `operationId` la retry); `get_deal` rămâne citirea de verificare înainte și după.
- **Configurare CRM**: capabilitățile tipurilor, vizibilitatea câmpurilor, etapele, stilul, regulile comerciale, echipele, calendarul, biblioteca și trainingul au instrumente dedicate dacă sunt disponibile live. Folosește `create_crm_pipeline_stage`, `update_crm_pipeline_stage`, `set_crm_business_profile`, `update_crm_configuration`; nu substitui numele legacy. Pentru capacitate și nominalizarea User CRM verifică instrumentele actuale sau pagina de setări. Întrebările paginii publice de întâlniri sunt distincte de formularele rezervărilor de eveniment.
...
După scriere: `get_reservations_overview` / `get_game_details` / `list_portal_games` / `get_crm_funnel` confirmă. Pentru deal-uri: `get_deal`; pentru oferte: `list_sales_quotes(dealId)` și apoi `get_sales_quote(quoteId)`. Succes la tool = salvat — nu repeta, spune userului să dea refresh. Audit „cine a creat/anulat" → `jurnal_activitate` (categorii „Rezervări"/„Contracte"/„SERVICES_CRM").
