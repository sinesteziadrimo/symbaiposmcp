...
5. **Verifică disponibilitatea** (citire, ÎNTÂI): `check_game_availability(gameId, date, time, players, exclusive?)` sau `get_game_slots(gameId, date, players?)`. Detalii: `list_portal_games`, `get_game_details`.
