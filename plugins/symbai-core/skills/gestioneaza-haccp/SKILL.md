...
- **Incident + recall (lanț frig întrerupt).** „S-a stricat frigiderul peste noapte, era carne în el (lotul 312)." → (1) `create_haccp_incident(type:"Lanț frig", description:"...", severity:"high", productLotId:312)`; (2) ca să vezi cât de departe a ajuns marfa, `build_recall_report(lotId:312, direction:"full")` — îți dă loturile în aval (în ce s-a transformat / unde a ajuns) + produsele distincte afectate, ca să decizi ce retragi. Când e rezolvat: `resolve_haccp_incident(incidentId:…, resolution:"am aruncat lotul, am reparat compresorul", correctiveAction:"verificare zilnică temperatură + senzor cu alarmă")`.
...
- **Severitate cu cap.** `high` = risc pentru sănătate (lanț frig pe carne/lactate, contaminare) → leagă-l de un recall; `medium` = pierdere de produs; `low` = rezolvare internă. La orice incident cu lot afectat, oferă `build_recall_report` ca să vadă impactul real.
...
- **Permisiune**: scrierile (loghează temperatură, incident, bifează curățenie, răcire) cer modulul **Setări & Configurare** (`setari`) pe token; citirile + recall merg mereu. „Permisiune insuficientă" → portal Hub → Acces AI → bifează modulul.
