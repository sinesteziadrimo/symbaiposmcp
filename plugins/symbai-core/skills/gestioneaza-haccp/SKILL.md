...
- **Cold-chain release.** „Pot livra lotul refrigerat 312?" → `get_cold_chain_release_readiness(lotId:312)` înainte de orice promisiune. Dacă răspunsul are blockers (temperaturi insuficiente, CCP/QC picat, incident HACCP deschis, hold activ), spui clar că lotul nu trebuie eliberat încă și direcționezi la fluxul de calitate (`productie-flux`) pentru dosar/decizie QA.
