...
- **Publicare/ștergere rămasă într-o stare ambiguă**: dacă rețeaua sau Meta a picat exact în timpul publicării, ștergerii sau modificării unei reclame, sistemul NU re-încearcă singur — ca să nu dubleze reclama sau cheltuiala. Campania rămâne un scurt interval blocată „în lucru" (operațiile noi primesc „are deja o operație în curs" — așteaptă până la ~15 minute), apoi e marcată **eșuată** și cere reluare MANUALĂ. La „de ce nu s-a publicat reclama": `get_ad_campaign_status(campaignId)` reconciliază starea cu Meta și îți dă motivul real; abia după ce vezi starea reiei manual operația (re-publicare / re-ștergere). Nu raporta „s-a publicat" cât timp starea e ambiguă sau `error`.
