...
3. **`apply_b2b_order_plan(orderId, warehouseId, approvalToken, orderMaterials:<da/nu>, addShift:<da/nu>, allowPartial:<da/nu>)`** = execută verificarea aprobată, cu aceleași `dateFrom` și `selection`. Trimite opțiunile explicit; nu lăsa implicitele să adauge materiale sau o tură nealese de om. Pentru achiziții transmite și `procurementPreviewToken` când este cerut. Dacă adaugă o tură și întoarce `necesitaReaprobare:true`, creează **numai tura**: arată noul plan și, după acordul pentru el, reapelează cu noul token și `addShift:false`. Abia aplicarea finală creează loturile și eventual achizițiile draft. Un token expirat sau un plan schimbat cere o verificare nouă.
