...
description: Comenzile B2B și necesarul rețelei proprii (fabrică/depozit central → magazine, francize, clienți firmă) — surse, stoc, forecast, planificare, picking, colete/QR, livrări parțiale în tranșe cu aprobare, ridicare fără șofer, rute, rampe, aviz și factură. La „ce comandă magazinele", „planifică comanda B2B", „livrăm doar o parte", „două mașini sau zile diferite", „eticheta fiecărui colet", „fă avizul/factura". Comenzi CĂTRE furnizori → comanda-furnizor.
---
...
Pentru ridicare din depozit, comenzi fără termen, colete și predare fizică citește [depozit-colete-predare](../../knowledge/depozit-colete-predare.md). Ziua ridicării, planificarea și termenul contractual sunt distincte.
Pentru livrare parțială, rest de livrat, mai multe mașini/zile și aprobări citește [b2b-livrari-partiale](../../knowledge/b2b-livrari-partiale.md): `get_b2b_delivery_split` → `preview_b2b_delivery_split` → `apply_b2b_delivery_split`. Citește toate paginile înainte de acceptare; folosește colete întregi dacă marfa este deja ambalată. Începe cu catalogul live: ghidul nu garantează disponibilitatea în versiunea instalată.
...
3. **Identifică firma și punctul**: ia `clientId` din `list_b2b_clients`, după nume și CUI, inclusiv pentru asocierea `partners` dintr-o integrare. Un identificator istoric dintr-un export poate fi diferit. Dacă răspunsul oferă `canonicalClientId`, folosește-l la următoarele apeluri; la modificări reia cu identificatorul indicat. Nu confunda firma cu punctul de livrare sau gestiunea.
4. **Înainte de necesar**: `get_b2b_distribution_network_setup` îți spune dacă brandul lucrează cu fabrică sau doar cu depozit central, ce punct este magazin propriu/franciză/B2B, zilele de livrare și sursa fiecărui produs. `depotId` este DESTINAȚIA din `list_b2b_client_depots`; nu este gestiunea-sursă.
5. **Starea reală**: ciornă → confirmare → producție doar pentru necesarul neacoperit → pregătire → ambalare → predare fizică → recepție. Documentele urmează politica configurată și pot apărea în momente diferite. Stocul nu confirmă automat ciorna, iar avizul/factura nu dovedesc predarea. Pentru tranșe verifică toate livrările legate. Magazinele proprii sunt transfer intern; francizele/B2B sunt flux comercial între firme.
...
| „fă avizul / emite factura pe comanda X" | `get_b2b_order_documents(orderId)` → emite documentul permis de regulă; `accept_b2b_aviz` numai după confirmarea reală a primirii de către client | `/b2b-orders?order=<id>` |
...
1. **`plan_b2b_order(orderId, warehouseId)`** = preview (nu scrie nimic). Alege gestiunea autorizată a fabricii; nu folosi punctul de livrare drept gestiune. Întoarce: verdictul (poate livra la termen?), costul/marja, materiile sub minim cu furnizor+ETA, gâtuirile pe echipamente, tura suplimentară propusă, câte loturi se creează, produsele neproducibile (fără rețetă), `approvalToken` și tokenul de aprovizionare când există. Arată verificarea și explică deciziile; dacă sumarul trunchiază lipsurile/zilele, consultă detaliile înainte de aprobarea întregului plan.
2. Obține **DA-ul** userului pe decizii: generăm comenzi furnizor pentru lipsuri? adăugăm tura suplimentară?
3. **`apply_b2b_order_plan(orderId, warehouseId, approvalToken, orderMaterials:<da/nu>, addShift:<da/nu>, allowPartial:<da/nu>)`** = execută verificarea aprobată, cu aceleași `dateFrom`, `selection` și `planningOptions`. Trimite opțiunile explicit; nu lăsa implicitele să adauge materiale sau o tură nealese de om. Pentru achiziții transmite și `procurementPreviewToken` când este cerut. Dacă adaugă o tură și întoarce `necesitaReaprobare:true`, creează **numai tura**: arată noul plan și, după acordul pentru el, reapelează cu noul token și `addShift:false`. Abia aplicarea finală creează loturile și eventual achizițiile draft. Un token expirat sau un plan schimbat cere o verificare nouă.
...
- **`apply_b2b_order_plan` SCRIE real — confirm-first.** Creează loturi de producție, poate genera comenzi furnizor draft și o tură suplimentară. Întâi `plan_b2b_order` (preview), arată-i userului ce se întâmplă și ce costă, folosește acordul explicit existent sau cere decizia care lipsește, apoi `apply_*`. Nu sări direct la apply.
...
- **„De ce nu pot factura?"** Citește motivul real cu `get_b2b_order_documents(orderId)`. Dacă lipsește acceptarea avizului, `accept_b2b_aviz` se folosește numai după confirmarea reală a primirii de către client. O derogare de facturare prin `force:true` și `confirm:true` cere intenția explicită a utilizatorului și dreptul aferent; nu este remediul automat pentru o regulă de recepție.
...
## Planificare simplă, asistată și modificări

Citește [ghidul comun pentru asistenți](../../knowledge/planificare-asistenti.md). `planningOptions` se păstrează identic la preview și apply; stocul și personalul lipsă nu blochează planificarea. Pentru fiecare operație poți indica manual echipamentul, durata și ora. `orderMaterials` și `addShift` sunt false implicit: aplică separat numai efectele autorizate. Acordul deja dat pentru aceeași propunere nu se cere din nou.
