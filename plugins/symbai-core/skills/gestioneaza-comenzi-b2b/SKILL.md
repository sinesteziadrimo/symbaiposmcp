...
- Modulul stă pe **`/b2b-orders`**, cu 7 tab-uri adresabile cu **`?tab=`**: `orders` (implicit, lista comenzilor), `clients`, `catalog`, `picking`, `routes` (dispeceratul: rute + rampe; `&view=timeline` = tabla Plan Încărcare), `delivery`, `analytics`. Du-te direct la tab: ex. `/b2b-orders?tab=picking`.
...
| „împarte comenzile de mâine pe mașini / planifică rutele" | `plan_b2b_delivery_routes(runDate)` (preview, arăți userului) → `apply_b2b_route_plan(runDate)` | `/b2b-orders?tab=routes` |
