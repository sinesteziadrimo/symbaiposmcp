...
## Înainte de orice
Pentru magazine, hoteluri și parcuri, citește **[website-teme-editoriale.md](../../knowledge/website-teme-editoriale.md)**: compoziții fotografice, capitole manuale, configurare pe brand și ancore pentru navigare. Verifică disponibilitatea în catalogul live înainte de aplicare.

Pentru fabrici alimentare/nealimentare, animații create de agent sau pictograme proprii de ingrediente, citește **[website-fabrici-animatii-proprii.md](../../knowledge/website-fabrici-animatii-proprii.md)**. Folosește identitățile reale și websiteId exact, verifică schema live și transferă imaginile prin prepare_website_image_upload → PUT → finish_website_image_upload.

Pentru restaurante, cafenele, baruri, pizzerii și evenimente citește mai întâi **[website-restaurante-spectaculoase.md](../../knowledge/website-restaurante-spectaculoase.md)**. Alege identitatea prin `list_website_templates` și experiențele prin `list_website_visual_experiences`; citește schema live, verifică datele reale și previzualizează înainte de salvare. Ghidul include prepararea din cardurile meniului de comandă, setările globale/pe componente/pe produse și evitarea rețetelor inventate. Nu porni direct cu HTML personalizat pentru aceste experiențe native.
...
| „pune un hero cu index de locatii / orase / destinatii care schimba poza" | `add_website_section(type:"index-hero", config:{eyebrow:"Hotels",items:[{label:"New York",image,url}],height:"screen",align:"left",ctaText,ctaUrl})` |
...
| „pune o listă de servicii lângă poză / valori cu linkuri" | `add_website_section(type:"split-list", config:{title,image,imagePosition:"right",items:[{title,description,linkText,linkUrl}]})` |
| „pune un carusel de destinații / experiențe / povești" | `add_website_section(type:"content-carousel", config:{title,imageRatio:"16/9",items:[{image,title,description,linkText,linkUrl}]})` |
| „pune hartă cu locațiile / find a restaurant / filtre pe oraș" | `add_website_section(type:"store-locator", config:{title,locations:[{name,address,city,region,hours,phone,mapQuery,directionsUrl}]})` |
| „arată cum funcționează / pașii comenzii / abonamentului" | `add_website_section(type:"process-steps", config:{title,layout:"horizontal",numberStyle:"circle",steps:[{title,description}]})` |
| „pune butoane App Store / Google Play / descarcă aplicația" | `add_website_section(type:"app-download", config:{title,subtitle,appStoreUrl,googlePlayUrl,stores?,image?})` sau `set_website_page_content` daca refaci pagina; verifica read-back + screenshot |
| „pune un meniu restaurant de prezentare / sample menu / filtre Vegan" | Pentru meniu STATIC de design: `add_website_section(type:"menu-list", config:{layout:"grid"|"list",items:[...],filters:true,filterTags:["v","vg","gf"]})`; pentru meniu live sincronizat citește catalogul POS și folosește componenta legată la produse |
...
| „schimbă culorile / aspectul" | `set_website_theme(primaryColor, secondaryColor, accentColor, backgroundColor, textColor, fontFamily, headingFont, siteName, tagline, scrollReveal?)`; apoi `get_website_page` ca să vezi `global` actualizat |
| „vreau animație când derulez / site-ul să nu fie static" | `set_website_theme(scrollReveal:true)` pentru reveal global pe secțiuni editoriale; verifică în browser desktop + mobil, mai ales dacă există elemente sticky/fixed |
| „vreau poza tăiată diagonal lângă text / secțiune split premium" | `add_website_section(type:"text-image", config:{title,html,imageUrl,imageMask:"diagonal",imagePosition:"right"})`; verifică vizual că muchia oblică este spre text |
