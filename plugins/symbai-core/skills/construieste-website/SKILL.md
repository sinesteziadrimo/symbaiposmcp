...
3. **Replică** prin tool-urile MCP, mapând `sourceBrief.sections` pe `set_hero` / `add_website_section` / `set_website_page_content` / `upsert_custom_website_component`, `sourceBrief.navigation` pe `update_website_navigation(items[], navbarSettings{})`, `sourceBrief.branding` pe `set_website_theme` + logo, iar footerul pe `set_website_footer`. Pentru catalog: produse → categorii → filtre → PDP → legale. După fiecare scriere de pagină/temă/navbar, confirmă cu `get_website_page`.
...
- **Hero real:** toate slide-urile, imaginea fiecărui slide, culoarea cardului/textului, punctele/săgețile, CTA-ul și destinația fiecărui slide.
...
| „meniul de sus să arate categoriile reale" | `update_website_navigation(rebuildCategoriesFromCatalog:true)` (sau `items[]` pentru control); pentru buton/search/login/transparent/sticky folosește `navbarSettings{}` |
...
| „schimbă culorile / aspectul" | `set_website_theme(primaryColor, secondaryColor, accentColor, fontFamily, headingFont, siteName, tagline)`; apoi `get_website_page` ca să vezi `global` actualizat |
