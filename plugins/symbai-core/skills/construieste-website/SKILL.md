...
- `menu-list` pentru meniuri restaurant STATICE de prezentare/clone (`layout:"grid"|"list"`, `items[]` sau `categories[]`, pret, meta/kcal, badge-uri, tag-uri dietetice, nivel iute, culori dark/light). Spune clar ca nu e sincronizat cu POS; pentru meniu live foloseste catalogul real / `menu-section`.
...
| „pune un meniu restaurant de prezentare / sample menu" | Pentru meniu STATIC de design: `add_website_section(type:"menu-list", config:{layout:"grid"|"list",items:[...]})`; pentru meniu live sincronizat citește catalogul POS și folosește componenta legată la produse |
