...
| „Operațiile pentru produsul Y / fă-mi fluxul" | Doar **fabrică** → citește fluxul cu `get_flow_graph` dacă există, apoi editează DOAR semantica (`build_complete_flow`, `add_flow_operation`, `add_operation_dependency`, materiale/output-uri/QC). După editare: `auto_arrange_diagram(flowVersionId)` → `validate_flow_consistency`. Nu seta coordonate x/y. |
...
| „Explică-mi / arată-mi fluxul unui produs" | Fabrică: deschide diagrama în `/fluxuri-tehnologice` și apasă **„Explică-mi"** (tur ghidat pas cu pas, de la materia primă la produsul finit). Diagrama se aranjează singură („Aranjează automat"), cu benzi pe zonă/echipament. |
