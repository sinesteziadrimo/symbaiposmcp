...
3. **Cum afli pe care e clientul** — în ordinea asta:
   ...
   - Sau confirmă vizual: dacă în meniu vede **Panou Fabrică**, **Enterprise Readiness**, **Execuție Producție**, **Planificare MPS** sau **Comenzi B2B** → e pe **fabrică**. Dacă vede doar pagina **Producție** (`/productie-evenimente`) → e pe **restaurant/simplu**.
...
| „De unde vine / unde a ajuns lotul / recall" | `exec_trace_lot_origin` (`lotId`) + `exec_trace_lot_destination` (`lotId`); pentru clienți expuși: `trace_recall_to_customers(lotId)`; pagina `/loturi-wip` tab Genealogie. În demo ARCA există și probe martor + documente `ARCA-TRACE-*` cu urme exacte `lot_outbound_traces`; folosește-le ca dovadă de audit/recall, nu ca instrucțiune de ștergere sau simulare manuală. |
