...
Pentru colete și borderouri citește [depozit-colete-predare](../../knowledge/depozit-colete-predare.md), pentru GLS [gls-magazin-online](../../knowledge/gls-magazin-online.md), iar pentru vitrine cu mai multe restaurante [meniu-partajat-livrari](../../knowledge/meniu-partajat-livrari.md). Verifică pe ce conexiune sunt oferite uneltele; nu transfera identități între platforma de livrare și POS.
...
- **După predare nu anula și nu refuza direct comanda.** O notificare de anulare de la platformă poate privi banii, fără ca mâncarea să fi revenit. Citește detaliile și istoricul: dacă apare anulare după predare, păstrează nota și consumul; tratează separat rambursarea și orice retur fizic confirmat. Nu modifica statusul sau stocul ca să ascunzi neconcordanța. Verifică funcțiile disponibile în versiunea live.
...
- **Livratorii costă în plus.** Fiecare angajat bifat livrator se taxează nominal (modul Livrator), iar marcarea cu `set_employee_as_driver` se reflectă automat în facturarea abonamentului. Spune-i userului când marchezi pe cineva.
...
- **Produse nemapate de pe platforme** (vândute pe Wolt/Glovo/Bolt fără legătură cu un produs Symbai, deci fără scădere din stoc): `list_unmapped_delivery_products` → `list_unmapped_delivery_product_occurrences` → `list_delivery_channel_menu_products` → `map_delivery_product_alias` cu `productId` și, pentru comenzile deja primite, `applyToHistory:true`; `reprocessConsumption:true` numai dacă reprocesarea nu este oprită de proprietar. O legătură greșită se scoate cu `delete_delivery_product_alias`.
...
- **Meniu platforme: full sync rar, update mic când se poate.** Full sync-ul de meniu la Glovo are o limită de siguranță pe zi per canal (peste ea trebuie `force:true` — folosește-l doar justificat). Dacă totalurile Glovo par greșite, verifică setarea canalului Glovo privind interpretarea prețului (preț pe linie vs preț unitar) în `/channels?tab=integrations`. Pentru Wolt, meniul poate trimite alergeni și ferestrele de disponibilitate săptămânală din datele produselor/canalului.
- **Disponibilitatea programată are sursa ei separată.** Pentru ferestre zi+oră folosește programările de disponibilitate (`list_availability_schedules` / `create_availability_schedule`): Wolt primește fereastra nativă la următorul sync de meniu, iar Glovo este comutat prin update mic per fereastră. Nu retrimite tot meniul doar ca să închizi/deschizi un produs pe ore.
