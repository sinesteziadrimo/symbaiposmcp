...
## Fluxul (pasi numerotati cu tool-urile MCP)
...
2. **Dacă NU există casierii:** trimite userul la `/finance/cash-registers` cu `gaseste_in_aplicatie("configurare casierii")` → alege modul de organizare (firmă / locație / brand×locație) → „Regenerează casieriile". E o configurare de o singură dată.
