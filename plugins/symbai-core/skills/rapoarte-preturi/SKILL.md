...
- Dacă `get_pnl`, `get_product_pnl` sau un raport de stoc întoarce o eroare de sursă/configurație, **nu transforma eroarea în 0** și nu continua analiza ca și cum cifra ar fi reală. Arată blockerul exact, verifică `get_pnl_config`/sursele și reapelează după corectură.
...
- **Livrările (Glovo/Wolt/Bolt/Tazz) se costează teoretic**, din rețetă, nu din loturile chiar consumate. Dacă s-a corectat prețul de pe o factură veche, corectura se vede la vânzările din sală după recalcularea perioadei, dar **nu** și la livrări — pentru ele se folosește recalcularea pe un singur produs, din pagina Consum Zilnic (`/daily-consumption`).
