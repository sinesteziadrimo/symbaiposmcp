...
3. **Citește ÎNTÂI starea curentă**: `get_portal_config(brandId)` întoarce TOATĂ configurarea (tip business, culori, texte, funcționalități, afișare, config meniu, **config QR**). Citește înainte de orice scriere — ca să nu strici ce e deja setat și ca să-i spui userului ce e acum vs. ce schimbi.
...
| „ce am acum pe portal / cum e configurat / ce nivel QR am" | `get_portal_config` (citire — include și config QR + portalMenuConfig) |
