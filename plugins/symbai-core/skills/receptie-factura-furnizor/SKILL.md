...
- **„Vreau să anulez/modific un NIR postat."** Urmează [corecția sau anularea nominală](../../knowledge/corectare-receptii-mcp.md), cu previzualizare și verificarea documentului. Pentru anulare, începe cu `preview_inventory_document_cancellation` din fluxul canonic; `diagnose_inventory_document_reversal` oferă dovezi suplimentare. Acceptarea impactului asupra loturilor folosite nu este un refuz și nu cere automat anularea consumurilor din aval. Dacă o ciornă generată blochează operația, abandoneaz-o prin fluxul dedicat în scopul autorizat și reia previzualizarea. Consumul anterior nu este automat un blocaj pentru corecția nominală din restaurant, care poate regulariza diferențele. Respectă refuzul concret și limitele perioadelor contabile; nu compensa printr-o ajustare care ascunde urma.
