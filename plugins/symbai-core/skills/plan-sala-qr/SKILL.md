...
| „în zona asta să nu iasă nota de plată / doar bon fiscal" | `list_zone_routing` → `set_zone_routing` | `zoneId` + `billPrintDisabled: true` (câștigă peste imprimanta ospătarului, pe web și în aplicația Symbai POS). `billPrinterId` gol NU oprește nota. Revii cu `billPrintDisabled: false`. |
...
- **Culoarea raionului se editează în designer** (`/plan-sala`, pe raion) — utilă ca ospătarii să-și recunoască dintr-o privire mesele; nu e nevoie să recreezi raionul ca să-i schimbi culoarea.
- **Auto-anularea comenzilor QR neconfirmate**: dacă niciun ospătar nu acceptă comanda QR în fereastra setată, comanda **expiră singură** (clientul vede fereastra chiar la confirmarea comenzii). Se setează în **Configurare Platformă Clienți Web → tab QR**: fereastra în minute, implicit 15; 0 = oprit. La „de ce s-a anulat singură comanda QR / de ce a dispărut", verifică întâi setarea asta.
